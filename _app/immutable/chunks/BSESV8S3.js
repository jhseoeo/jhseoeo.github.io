import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{s,f as tn,c as a,r as t,n as pn}from"./CVx5jffJ.js";import{f as u,a as c}from"./2cXyNWGb.js";import{h as p}from"./D6IIVJDJ.js";import{C as l}from"./BQl3WXse.js";import{I as on}from"./BVXqo7x9.js";const en={title:"DDD와 함께 TDD, BDD 사용하기",date:"2023-08-11T00:00:00.000Z",excerpt:"TDD, BDD, and DDD",categories:["Golang","Backend","Architecture","Domain Driven Design"],coverImage:"/post_img/Backend/Architecture/DDD/cover.png",coverWidth:16,coverHeight:9,indexed:!1,exposed:!0},{title:Tn,date:Sn,excerpt:En,categories:Pn,coverImage:In,coverWidth:Gn,coverHeight:Bn,indexed:Nn,exposed:$n}=en;var cn=u('<pre class="language-go"><!></pre>'),un=u('<pre class="language-go"><!></pre>'),ln=u('<pre class="language-go"><!></pre>'),kn=u('<pre class="language-go"><!></pre>'),rn=u('<pre class="language-go"><!></pre>'),dn=u('<pre class="language-go"><!></pre>'),gn=u('<pre class="language-go"><!></pre>'),fn=u('<pre class="language-go"><!></pre>'),hn=u('<pre class="language-go"><!></pre>'),mn=u('<pre class="language-go"><!></pre>'),_n=u(`<p>지금까지 DDD의 핵심적인 개념에 대해 알아보았다.
하지만 DDD 외에도 TDD, BDD 등 DDD와 함께 사용할 수 있는 여러 개발 방법론이 존재한다.
본 포스트에서는 이에 대해 알아보고, DDD와 함께 사용할 수 있는 방법에 대해 알아보려 한다.</p> <br/><br/> <h2 id="tdd"><a aria-hidden="true" tabindex="-1" href="#tdd"><span class="icon icon-link"></span></a>TDD</h2> <hr/> <p>TDD(Test Driven Development)는 소프트웨어를 개발하기 이전에 비즈니스 요구사항을 충족시키는 테스트를 먼저 작성하는 개발 방법론이다.
코드를 작성할 때 테스트케이스를 지속적으로 업데이트해야 하고, 테스트케이스를 통과하는 코드를 작성해야 한다.</p> <p>가령 TDD를 사용하여 애플리케이션에 새로운 기능을 추가하는 작업을 수행한다고 할 때, 다음과 같은 절차를 따른다.</p> <ol><li><p>코드를 작성하기 전에 테스트케이스를 작성한다. <em>“API 유저인 내가 홈 화면에서 <code>/balance</code> 엔드포인트로 요청을 날려서 모든 계정의 잔고를 확인할 수 있어야 한다”</em>처럼 유저 스토리를 기반으로 테스트케이스를 작성할 수 있다.
또는 <em>“Given API 사용자, When <code>/balance</code> 호출, Then 모든 통장의 잔고를 확인할 수 있음”</em>과 같이 Given-When-Then(GWT) 패턴으로 작성할 수도 있다.</p> <p>이 단계에서는 단 한 줄의 코드도 작성하지 않으며, 비즈니스 요구사항에 초점을 맞추어야 한다.</p></li> <li><p>테스트를 실행한다. 코드를 작성하지 않았기 때문에, 테스트는 당연히 실패할 것이다.
이 단계는 테스트케이스와 테스트 프레임워크가 잘 설정되었는지 확인하는 단계이다.</p></li> <li><p>테스트를 통과하는 최소한의 코드를 작성한다.
이 단계에서 작성되는 코드는 특별히 예쁘고 깔끔할 필요 없다! 그저 테스트를 통과하기만 하면 된다.
비즈니스 불변성을 위반하지 않는 선에서 테스트를 통과하기 위한 모든 방법을 동원한다.</p></li> <li><p>테스트를 재실행한다. 새 테스트뿐 아니라, 기존의 테스트도 모두 통과해야 한다.
즉, 새로운 코드가 기존의 코드를 망가뜨리지 않았는지 확인한다.</p></li> <li><p>코드를 리팩토링한다. 3단계에서 작성한 예쁘지 않은 코드를 예쁘게 리팩토링해주면 된다.
틈틈이 테스트를 실행하면서 코드가 잘 동작하는지 확인한다.</p></li></ol> <p>이러한 절차를 반복하는 것이 TDD의 핵심이다.
TDD는 새로운 기능을 추가할 때 뿐만 아니라, 기존의 코드를 수정할 때에도 사용할 수 있다. 즉, 레거시 코드를 개선하거나 디버깅할 때에도 유용하다.</p> <br/> <p>다음과 같은 Given-When-Then 테스트케이스가 있다고 가정해보자.</p> <p>제목: <em>고객이 쿠키를 구매하면, 이메일 영수증을 받는다.</em></p> <p>그렇다면 다음과 같은 합격 기준을 정의할 수 있다.</p> <ul><li>사용자가 쿠키를 구매할 때 쿠키 재고가 충분하다면 결제가 완료된 이후 잠시 뒤 이메일 영수증을 받는다.</li> <li>사용자가 쿠키를 구매할 때 쿠키 재고가 충분하지 않다면 계산원에게 쿠키 재고가 부족하다는 알림을 보낸다.</li> <li>사용자가 쿠키를 구매할 때 쿠키 재고가 충분하지만 카드 결제가 실패한다면 계산원에게 카드 결제가 실패했다는 알림을 보낸다.</li> <li>결제가 완료되었지만 이메일 전송에 실패한 경우, 계산원에게 이메일 전송이 실패했다는 알림을 보낸다.</li></ul> <p>이제 TDD 절차에 따라 기능을 구현해볼 것이다.</p> <br/><br/> <h3 id="테스트-추가"><a aria-hidden="true" tabindex="-1" href="#테스트-추가"><span class="icon icon-link"></span></a>테스트 추가</h3> <p><code>cookies.go</code> 파일 및 <code>cookies_test.go</code> 파일을 생성한다.</p> <pre class="language-text"><!></pre> <p>go는 테스트 파일의 이름이 <code>_test.go</code>로 끝나는 파일을 테스트 파일로 인식한다. 또한 빌드 시 테스트 파일은 무시된다. golang에서의 테스트에 관련한 자세한 내용은 <a href="https://jhseoeo.github.io/posts/post/Go/Golang%20Basics/GO13" rel="nofollow">여기</a>를 참고하자.</p> <p>TDD 절차에 따라, 테스트를 먼저 작성해 보자! <code>cookies_test.go</code> 파일에 다음과 같이 테스트케이스를 작성한다.</p> <pre class="language-go"><!></pre> <p>위 코드에서 확인할 수 있듯 테스트케이스의 이름에 합격 기준이 명시되었다.
바로 이 부분에서 TDD와 DDD가 잘 어울린다는 것을 알 수 있다. TDD의 합격 기준 자체가 도메인 전문가로부터 나온 셈이며, 이 내용이 테스트케이스에 명시됨으로써 도메인 전문가와 개발자 사이의 의사소통이 원활해진다.</p> <p>Go에서는 테스트가 비어 있으면 통과한 것으로 간주하기 때문에, <code>t.FailNow()</code>를 추가하여 테스트가 실패하도록 만들었다.
이와 같이, 원칙적으로 아직 작성하지 않은 테스트는 실패해야 한다.</p> <p>그럼 테스트를 실행해보자!</p> <pre class="language-bash"><!></pre> <p>테스트가 성공적으로 실패했다..! 다음 단계로 넘어가보자.</p> <br/><br/> <h3 id="코드-작성"><a aria-hidden="true" tabindex="-1" href="#코드-작성"><span class="icon icon-link"></span></a>코드 작성</h3> <p>지금까지 테스트만을 작성했고, 코드는 한 줄도 작성하지 않았다.
이제 할 일은 <code>cookies.go</code> 파일에 테스트를 통과할 수 있는 최소한의 코드를 작성하는 것이다.
거듭 언급하지만, 이 단계에서 코드를 특별히 예쁘게 작성할 필요는 없다.</p> <!> <br/><br/> <h3 id="테스트-실행"><a aria-hidden="true" tabindex="-1" href="#테스트-실행"><span class="icon icon-link"></span></a>테스트 실행</h3> <p><code>PurchaseCookies</code> 메서드는 몇 가지 인터페이스를 통해 의존성을 주입받는다.
이러한 경우 인터페이스를 Mocking하여 테스트를 쉽게 작성할 수 있다.
이를테면 이메일 전송시 오류가 발생하는 상황을 테스트하기 위해, <code>EmailSender</code> 인터페이스에 대한 Mock을 만들어 <code>SendEmailReceipt</code> 메서드가 오류를 반환하도록 만들 수 있다.</p> <p>Golang 개발팀에서는 Mocking을 위해 <a href="https://github.com/golang/mock" rel="nofollow">gomock</a> 프레임워크를 제공한다.
이 프레임워크를 이용하여 mock을 만들어보자.</p> <p>먼저, <code>gomock</code>을 설치한다.</p> <pre class="language-bash"><!></pre> <p>이후 프로젝트 루트 디렉토리에 <code>gen.go</code> 파일을 생성하고, 다음과 같이 작성한다.</p> <pre class="language-go"><!></pre> <p>이제 <code>go generate ./...</code> 명령어를 실행하면, <code>mocks</code> 디렉토리에 Mock이 생성된 것을 확인할 수 있다.</p> <pre class="language-text"><!></pre> <p>마지막으로, 생성된 mock을 활용하여, <code>cookies_test.go</code> 파일을 다음과 같이 수정한다.</p> <!> <p>위 테스트 코드에서는 <code>gomock</code>을 통해 생성된 Mock을 사용하여 각 interface를 충족시키는 mock을 만들고, <code>NewCookieService</code> 메서드를 통해 <code>CookieService</code>를 생성한다.
또한 <code>gomock</code>의 기능을 이용하여, mock이 지정된 파라미터를 받았을 때 어떤 동작을 수행해야 하는지를 지정한다.
이후 <code>PurchaseCookies</code> 메서드를 호출하고, 동작이 예상대로 수행되는지 확인할 수 있다..</p> <p><code>go test</code> 명령어를 통해 이 테스트를 실행하면 통과할 것이다. 테스트 설명에 명시된 기준을 충족시킨다.
하지만 코드에 몇 가지 TODO 태그가 남아있는 것을 확인할 수 있다.
도메인 전문가와의 질의를 진행한 후 이 부분을 수정해보자.</p> <p>Q. 쿠키의 가격은 어떻게 결정되나요?<br/> A. 나중에 변경될 수 있지만 당장은 50센트로 고정되어 있습니다.</p> <p>Q. 쿠키 재고보다 더 많은 쿠키를 구매하려고 하면 어떻게 해야 할까요?
A. 일단 남아있는 재고만큼만 구매할 수 있도록 합니다.</p> <p>Q. 사용자의 카드 토큰은 어떻게 얻나요?
A. 카드 결제하면 뿅 나와요</p> <p>Q. 사용자의 이메일 주소는 어떻게 얻나요?
A. 이것도 결제하면 뿅 나와요</p> <p>서비스가 어떻게 동작해야 할지 더 잘 이해할 수 있게 되었다.
다만 TDD니까, 코드를 수정하기에 앞서 테스트를 먼저 작성해야 한다.
테스트를 작성하기 위해 <code>cookies_test.go</code> 파일에 다음과 같은 테스트 stub을 추가한다.</p> <!> <br/><br/> <h3 id="리팩토링"><a aria-hidden="true" tabindex="-1" href="#리팩토링"><span class="icon icon-link"></span></a>리팩토링</h3> <p>TDD의 마지막 단계는 리팩토링이다.
위에서 진행하였던, 도메인 전문가와의 질의 내용을 반영하여 코드를 수정할 것이다.</p> <p>일단 쿠키 가격이 50센트로 변경된다. 이를 반영하면 당연히 테스트는 실패할 것이다. 따라서 테스트 코드도 수정해준다. 당연한 얘기니 코드는 생략!</p> <p>다른 테스트들도 작성할 것이다. 먼저, 쿠키 재고보다 더 많은 쿠키를 구매하려고 하면 어떻게 해야 할지에 대한 테스트를 작성한다.</p> <!> <p>일단 이 테스트를 실행하면 성공할 것이다. 코드에서 쿠키 재고가 부족하면 에러를 반환하게끔 작성해두었기 때문.</p> <p>하지만 쿠키 재고가 있을 때는 남아있는 재고만큼만 구매할 수 있도록 하라는 요구사항이 있다. 이를 반영하여 코드를 수정해야 한다.</p> <!> <br/> <p>다음 단계의 테스트를 먼저 작성해보자. 카드 결제가 실패했을 때, 어떻게 처리할지에 대한 테스트이다.</p> <!> <p>테스트를 실행하면 다음과 같은 에러가 발생한다.</p> <pre class="language-text"><!></pre> <p>얼추 보니 코드에서 반환하는 에러 메시지와 테스트에서 기대하는 에러 메시지가 다르다.
코드에서는 에러를 한번 <code>fmt.Errorf</code>로 감싸서 반환하고 있다. 따라서 테스트에서 기대하는 에러 메시지를 수정하거나, 코드에서 <code>fmt.Errorf</code>를 제거하면 테스트가 성공할 것이다. 코드를 수정하는 것이 더 좋아보이니, 코드를 수정해보자.
이를 반영하여 코드를 수정하고, 다시 테스트를 실행해보자.</p> <pre class="language-go"><!></pre> <p>이후 테스트를 실행하면 성공할 것이다.</p> <pre class="language-text"><!></pre> <p>이제 코드를 리팩토링하는 단계이지만, 달리 리팩토링할 부분이 없으니 생략하고 다음 단계로 넘어간다.</p> <br/> <p>다음 단계의 테스트는 이메일 전송이 실패했을 때, 어떻게 처리할지에 대한 테스트이다.
먼저 테스트를 작성한다.</p> <pre class="language-go"><!></pre> <p>테스트를 실행하면 이전 단계와 동일한 에러가 발생한다. 해결 방법도 동일하다.
코드를 수정하거나, 테스트에서 기대하는 에러 메시지를 수정하면 된다. 마찬가지로 코드를 수정하는 것이 더 좋아보이니, 코드를 수정해보자.</p> <pre class="language-go"><!></pre> <p>테스트를 다시 실행하면 성공할 것이다.</p> <br/> <p>마지막 테스트를 작성해보자.</p> <!> <p>위 테스트를 실행하면 다음과 같이 실패할 것이다.</p> <pre class="language-text"><!></pre> <p>남아있는 쿠키 재고가 구매하려는 쿠키 재고보다 적을 때 처리하는 로직을 코드에 작성하지 않았기 때문에 테스트가 실패하는 것이다.
따라서 코드를 수정해보자.</p> <!> <p>테스트를 다시 실행하면 성공할 것이다.</p> <br/> <p>이제 모든 테스트 케이스가 통과하므로, <code>go test -cover</code> 명령을 실행해보자.</p> <pre class="language-text"><!></pre> <p>100% 커버리지를 달성한 것을 확인할 수 있다! 신난다🎉🎉</p> <br/> <p>다만 지금까지 작성한 테스트 코드에서 궁금증이 좀 생길 수 있다.
얼핏 봐도 상당한 코드 중복이 있다. 일정한 패턴이 보이는데, 적당히 묶어서 함수로 만들거나 하면 좋지 않을까?
이에 대한 대답은, 그래도 된다는 것이다. 실제로 Go에는 Table Test라고 하는 방식이 있어서, 테스트 코드를 더 깔끔하게 작성할 수 있다.</p> <p>하지만 이 책에서는 테스트 코드가 더 깔끔해지는 것보다는, 테스트 코드가 더 명확하게 보이는 것을 우선시한다.
테스트 코드는 그 자체로 일종의 문서 역할을 하기 때문에, 테스트 코드를 읽는 사람이 테스트 코드를 이해하기 쉽게 작성하는 것이 중요하다고 설명한다.</p> <br/> <p>아직 안 끝났다ㅠㅠ 아직 구현하지 않은 요구사항이 있다.</p> <p>코드에는 카드 토큰과 이메일 주소가 하드코딩되어 있는데, 이는 요청할 때 파라미터로 받아오게 될 것이다.
따라서, 함수 정의를 바꿔야 한다.</p> <pre class="language-go"><!></pre> <p>하지만 이렇게 변경하고 나면 기존에 작성한 테스트 코드들이 모두 실패할 것이다.
이에 맞게 테스트 코드를 수정해줄 필요가 있다.</p> <!> <p>이와 같이 테스트 코드를 수정해주면, 테스트가 다시 성공할 것이다.</p> <br/> <p>이 정도면 TDD로 개발하는 것에 대한 전반적인 감을 잡을 수 있을 것이다. 다음 단계인 BDD(Behavior-Driven Development)에 대해 알아보자.</p> <br/><br/> <h2 id="bdd"><a aria-hidden="true" tabindex="-1" href="#bdd"><span class="icon icon-link"></span></a>BDD</h2> <p>BDD는 TDD의 확장판같은 개념으로, 엔지니어-도메인 전문가-QA간의 긴밀한 협업을 가능하게 하는 개발 방법론이다.</p> <p>BDD의 목표는 Domain-Specific Language(DSL)을 통해 더 높은 수준의 추상화를 제공하는 것이며, 이는 테스트로 실행 가능하다.
가장 유명한 BDD 테스트 프레임워크는 <a href="https://cucumber.io/docs/gherkin/" rel="nofollow">Gherkin</a>과 <a href="https://cucumber.io/" rel="nofollow">Cucumber</a>이다.
Gherkin은 몇 개의 키워드로 이루어진 언어를 제공하며, Cucumber는 이 언어를 읽어 소프트웨어가 예상대로 작동하는지 검증한다.</p> <p><a href="https://github.com/go-bdd/gobdd" rel="nofollow"><code>go-bdd</code> 프레임워크</a>를 활용하여 BDD 테스트를 작성해보자. 먼저, go-bdd를 설치한다.</p> <pre class="language-bash"><!></pre> <p>이후 프로젝트 루트 디렉토리에 <code>features</code> 디렉토리를 생성하고, <code>features/add.feature</code> 파일을 생성한 후 다음과 같이 작성한다.</p> <pre class="language-gherkin"><!></pre> <p>그리고 <code>add_test.go</code> 파일을 생성하여 다음과 같이 작성한다.</p> <!> <p>위 코드에서 <code>add</code> 함수는 <code>bdd step</code> 함수인데, 함수 이름이 중요하다. <code>add.feature</code> 파일의 <code>When I add 3 and 6</code>이라는 문장을 만나면, <code>add</code> 함수가 실행된다.
만약 <code>add</code> 함수의 이름을 <code>sum</code>으로 변경한다면, <code>add.feature</code> 파일을 <code>When I sum 3 and 6</code>으로 변경해야 할 것이다.
이후 로직을 수행하고 나중에 <code>check</code>에서 불러올 수 있도록 컨텍스트에 저장한다.</p> <p><code>check</code> 함수는 실질적인 테스트를 수행하는 함수로, 검증은 이 함수에서 이루어진다.
다음으로 테스트 스위트를 생성하고, <code>add</code>와 <code>check</code> 함수를 등록한다.
이 테스트를 수행한다면 통과할 것이다.</p> <br/> <p>이와 같이 BDD는 자연어와 유사하여 직관성이 뛰어나지만, 테스트 코드를 작성하는 것이 번거로워진다.
...
<span class="token punctuation">&#125;</span></code>`),t(r);var d=s(r,8),W=a(d);p(W,()=>`<code class="language-bash">$ go <span class="token builtin class-name">test</span> ./<span class="token punctuation">..</span>.
...
FAIL    github.com/jhseoeo/Golang-DDD/chapter8  <span class="token number">0</span>.001s</code>`),t(d);var E=s(d,11);l(E,{children:(o,k)=>{var n=cn(),e=a(n);p(e,()=>`<code class="language-go"><span class="token keyword">package</span> chapter8
...
<span class="token punctuation">&#125;</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var g=s(E,13),j=a(g);p(j,()=>'<code class="language-bash">go <span class="token function">install</span> github.com/golang/mock/mockgen@v1.6.0</code>'),t(g);var f=s(g,4),q=a(f);p(q,()=>`<code class="language-go"><span class="token keyword">package</span> chapter8
...
<span class="token comment">//go:generate mockgen -package mocks -destination chapter8/mocks/cookies.go github.com/jhseoeo/Golang-DDD/chapter8 CookieStockChecker,CardCharger,EmailSender</span></code>`),t(f);var h=s(f,4),Q=a(h);p(Q,()=>`<code class="language-text">.
├── chapter8
│   └── mocks
│       └── cookies.go
├── cookies.go
...
├── gen.go
├── go.mod
└── go.sum</code>`),t(h);var P=s(h,4);l(P,{children:(o,k)=>{var n=un(),e=a(n);p(e,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">Test_CookiePurchases</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user tries to purchase a cookie and we have them in stock,
    when they tap their card, they get charged and then receive an email receipt a few moments later.&#96;</span><span class="token punctuation">,</span>
		<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			...
			<span class="token punctuation">&#125;</span>
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var I=s(P,16);l(I,{children:(o,k)=>{var n=ln(),e=a(n);p(e,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user tries to purchase a cookie and we don't have any in stock, we return an error to the cashier
   ...
   get an e-mail, but the transaction is still considered done.&#96;</span><span class="token punctuation">,</span>
	...
	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given someone wants to purchase more cookies than we have in stock we only charge them for the ones we do have&#96;</span><span class="token punctuation">,</span>
		<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			t<span class="token punctuation">.</span><span class="token function">FailNow</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var G=s(I,13);l(G,{children:(o,k)=>{var n=kn(),e=a(n);p(e,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user tries to purchase a cookie and we don't have any in stock, we return an error to the cashier
      so they can apologize to the customer.&#96;</span><span class="token punctuation">,</span>
		<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			ctrl <span class="token operator">:=</span> gomock<span class="token punctuation">.</span><span class="token function">NewController</span><span class="token punctuation">(</span>t<span class="token punctuation">)</span>
			e <span class="token operator">:=</span> mocks<span class="token punctuation">.</span><span class="token function">NewMockEmailSender</span><span class="token punctuation">(</span>ctrl<span class="token punctuation">)</span>
			...
			<span class="token punctuation">&#125;</span>
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var B=s(G,6);l(B,{children:(o,k)=>{var n=rn(),e=a(n);p(e,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token punctuation">(</span>c <span class="token operator">*</span>CookieService<span class="token punctuation">)</span> <span class="token function">PurchaseCookies</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> amountOfCookies <span class="token builtin">int</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var N=s(B,6);l(N,{children:(o,k)=>{var n=dn(),e=a(n);p(e,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user tries to purchase a cookie, we have them in stock, but their card gets declined, we return
	an error to the cashier so that we can ban the customer from the store.&#96;</span><span class="token punctuation">,</span>
			<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
				ctrl <span class="token operator">:=</span> gomock<span class="token punctuation">.</span><span class="token function">NewController</span><span class="token punctuation">(</span>t<span class="token punctuation">)</span>
				e <span class="token operator">:=</span> mocks<span class="token punctuation">.</span><span class="token function">NewMockEmailSender</span><span class="token punctuation">(</span>ctrl<span class="token punctuation">)</span>
				...
				<span class="token punctuation">&#125;</span>
			<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var m=s(N,4),z=a(m);p(z,()=>`<code class="language-text">--- FAIL: Test_CookiePurchases/Given_a_user_tries_to_purchase_a_cookie,_we_have_them_in_stock,_but_their_card_gets_declined,_we_return____an_error_to_the_cashier_so_that_we_can_ban_the_customer_from_the_store. (0.00s)
        cookies_test.go:92: expected error, got failed to charge card: some error</code>`),t(m);var _=s(m,4),H=a(_);p(H,()=>`<code class="language-go">	err <span class="token operator">:=</span> c<span class="token punctuation">.</span>cardCharger<span class="token punctuation">.</span><span class="token function">ChargeCard</span><span class="token punctuation">(</span>ctx<span class="token punctuation">,</span> <span class="token string">"some-token"</span><span class="token punctuation">,</span> cost<span class="token punctuation">)</span>
	<span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">&#123;</span>
		<span class="token keyword">return</span> err
	<span class="token punctuation">&#125;</span></code>`),t(_);var w=s(_,4),U=a(w);p(U,()=>'<code class="language-text">--- PASS: Test_CookiePurchases/Given_a_user_tries_to_purchase_a_cookie,_we_have_them_in_stock,_but_their_card_gets_declined,_we_return____an_error_to_the_cashier_so_that_we_can_ban_the_customer_from_the_store. (0.00s)</code>'),t(w);var b=s(w,8),Z=a(b);p(Z,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user purchases a cookie and we have them in stock, their card is charged successfully but we
	fail to send an email, we return a message to the cashier so they know can notify the customer that they will not
	get an e-mail, but the transaction is still considered done.&#96;</span><span class="token punctuation">,</span>
			<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
				ctrl <span class="token operator">:=</span> gomock<span class="token punctuation">.</span><span class="token function">NewController</span><span class="token punctuation">(</span>t<span class="token punctuation">)</span>
				...
				<span class="token keyword">if</span> err<span class="token punctuation">.</span><span class="token function">Error</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">!=</span> <span class="token string">"failed to send email"</span> <span class="token punctuation">&#123;</span>
					t<span class="token punctuation">.</span><span class="token function">Fatalf</span><span class="token punctuation">(</span><span class="token string">"expected error, got %v"</span><span class="token punctuation">,</span> err<span class="token punctuation">)</span>
				<span class="token punctuation">&#125;</span>
			<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(b);var v=s(b,4),J=a(v);p(J,()=>`<code class="language-go">	err <span class="token operator">:=</span> c<span class="token punctuation">.</span>emailSender<span class="token punctuation">.</span><span class="token function">SendEmailReceipt</span><span class="token punctuation">(</span>ctx<span class="token punctuation">,</span> <span class="token string">"some-email"</span><span class="token punctuation">)</span>
	<span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">&#123;</span>
		<span class="token keyword">return</span> err
	<span class="token punctuation">&#125;</span></code>`),t(v);var $=s(v,8);l($,{children:(o,k)=>{var n=gn(),e=a(n);p(e,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given someone wants to purchase more cookies than we have in stock we only charge them for the ones we do have&#96;</span><span class="token punctuation">,</span>
		<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			ctrl <span class="token operator">:=</span> gomock<span class="token punctuation">.</span><span class="token function">NewController</span><span class="token punctuation">(</span>t<span class="token punctuation">)</span>
			e <span class="token operator">:=</span> mocks<span class="token punctuation">.</span><span class="token function">NewMockEmailSender</span><span class="token punctuation">(</span>ctrl<span class="token punctuation">)</span>
			c <span class="token operator">:=</span> mocks<span class="token punctuation">.</span><span class="token function">NewMockCardCharger</span><span class="token punctuation">(</span>ctrl<span class="token punctuation">)</span>
			...
			<span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">&#123;</span>
				t<span class="token punctuation">.</span><span class="token function">Fatalf</span><span class="token punctuation">(</span><span class="token string">"unexpected error: %v"</span><span class="token punctuation">,</span> err<span class="token punctuation">)</span>
			<span class="token punctuation">&#125;</span>
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var C=s($,4),K=a(C);p(K,()=>`<code class="language-text">--- FAIL: Test_CookiePurchases (0.00s)
...
FAIL
exit status 1
FAIL    github.com/jhseoeo/Golang-DDD/chapter8  0.001s</code>`),t(C);var R=s(C,4);l(R,{children:(o,k)=>{var n=fn(),e=a(n);p(e,()=>`<code class="language-go">cookiesInStock <span class="token operator">:=</span> c<span class="token punctuation">.</span>cookieStockChecker<span class="token punctuation">.</span><span class="token function">AmountInStock</span><span class="token punctuation">(</span>ctx<span class="token punctuation">)</span>
	<span class="token keyword">if</span> cookiesInStock <span class="token operator">==</span> <span class="token number">0</span> <span class="token punctuation">&#123;</span>
		<span class="token keyword">return</span> errors<span class="token punctuation">.</span><span class="token function">New</span><span class="token punctuation">(</span><span class="token string">"no cookies in stock"</span><span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">if</span> cookiesInStock <span class="token operator">&lt;</span> amountOfCookies <span class="token punctuation">&#123;</span>
		amountOfCookies <span class="token operator">=</span> cookiesInStock
	<span class="token punctuation">&#125;</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var y=s(R,8),V=a(y);p(V,()=>`<code class="language-text">PASS
        github.com/jhseoeo/Golang-DDD/chapter8  coverage: 100.0% of statements
ok      github.com/jhseoeo/Golang-DDD/chapter8  0.002s</code>`),t(y);var x=s(y,16),Y=a(x);p(Y,()=>'<code class="language-go"><span class="token keyword">func</span> <span class="token punctuation">(</span>c <span class="token operator">*</span>CookieService<span class="token punctuation">)</span> <span class="token function">PurchaseCookies</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> amountOfCookies <span class="token builtin">int</span><span class="token punctuation">,</span> cardToken <span class="token builtin">string</span><span class="token punctuation">,</span> email <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span></code>'),t(x);var A=s(x,4);l(A,{children:(o,k)=>{var n=hn(),e=a(n);p(e,()=>`<code class="language-go">	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span><span class="token string">&#96;Given a user tries to purchase a cookie and we have them in stock,
    when they tap their card, they get charged and then receive an email receipt a few moments later.&#96;</span><span class="token punctuation">,</span>
		<span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
			ctrl <span class="token operator">:=</span> gomock<span class="token punctuation">.</span><span class="token function">NewController</span><span class="token punctuation">(</span>t<span class="token punctuation">)</span>
			e <span class="token operator">:=</span> mocks<span class="token punctuation">.</span><span class="token function">NewMockEmailSender</span><span class="token punctuation">(</span>ctrl<span class="token punctuation">)</span>
			...
			<span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">&#123;</span>
				t<span class="token punctuation">.</span><span class="token function">Fatalf</span><span class="token punctuation">(</span><span class="token string">"unexpected error: %v"</span><span class="token punctuation">,</span> err<span class="token punctuation">)</span>
			<span class="token punctuation">&#125;</span>
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var D=s(A,19),nn=a(D);p(nn,()=>'<code class="language-bash">go get github.com/go-bdd/gobdd</code>'),t(D);var T=s(D,4),sn=a(T);p(sn,()=>`<code class="language-gherkin"><span class="token feature"><span class="token keyword">Feature:</span><span class="token important"> Adding numbers</span>
        ...
        <span class="token atrule">Then</span> the result should equal 9</code>`),t(T);var F=s(T,4);l(F,{children:(o,k)=>{var n=mn(),e=a(n);p(e,()=>`<code class="language-go"><span class="token keyword">package</span> chapter8
...
<span class="token punctuation">&#125;</span></code>`),t(n),c(o,n)},$$slots:{default:!0}});var M=s(F,17),an=s(a(M),2);on(an,{alt:"Domain-Driven Design with Golang Cover",src:"https://learning.oreilly.com/covers/urn:orm:book:9781804613450/400w/"}),pn(3),t(M),c(O,S)}export{Rn as default,en as metadata};
