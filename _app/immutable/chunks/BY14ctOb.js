import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{s as n,f as es,c as s,r as a,n as pn}from"./CVx5jffJ.js";import{f as cs,a as us}from"./2cXyNWGb.js";import{h as t}from"./D6IIVJDJ.js";import{I as on}from"./BVXqo7x9.js";const ls={title:"테스트 작성",date:"2023-02-06T00:00:00.000Z",excerpt:"Writing Tests",categories:["Golang","Basic Golang"],coverImage:"/post_img/Go/Golang_basics/cover.png",coverWidth:16,coverHeight:9,indexed:!1,exposed:!0},{title:ms,date:ys,excerpt:hs,categories:ws,coverImage:vs,coverWidth:xs,coverHeight:_s,indexed:Ts,exposed:Ps}=ls;var is=cs(`<br/> <p>본 글은 Golang을 공부하며 주요 내용이라 생각되는 것들을 기록해둔 자료이며, Ubuntu 22.04 LTS 기준으로 작성되었습니다.</p> <br/><br/> <h2 id="introduction"><a aria-hidden="true" tabindex="-1" href="#introduction"><span class="icon icon-link"></span></a>Introduction</h2> <hr/> <p>정말 오랜만에 Golang 포스팅을 다시 하게 되었다. 학업에 치이다 보니 잠시 Golang을 멀리 했었는데, 오랜만에 쓰려니 어디까지 했었는지 감이 잘 안온다. 기존에 내가 썼던 글 다시 보고 최대한 감을 되찾아 써보려 한다.</p> <br/> <p>지난 20년간 테스트의 자동화는 코드 품질을 향상시키는 데 지대한 공헌을 했다.
...
이를 위해 테스트 가능한 코드를 작성하는 방법과, 이를 통해 코드 품질이 향상되는 이유에 대해 알아볼 것이다.</p> <br/><br/> <h2 id="the-basics-of-testing"><a aria-hidden="true" tabindex="-1" href="#the-basics-of-testing"><span class="icon icon-link"></span></a>The Basics of Testing</h2> <hr/> <p>Go의 테스트는 라이브러리와 도구로 나뉜다.
표준 라이브러리의 <code>testing</code> 패키지는 테스트를 작성하기 위한 타입과 함수를 제공한다.
한편 Go와 함께 번들로 제공되는 <code>gotest</code>라는 도구는 테스트를 실행하고 보고를 생성한다.
다른 언어와는 달리 Go의 테스트는 프로덕션 코드와 같은 디렉토리 및 같은 패키지에 배치한다.
테스트가 같은 패키지에 위치하기 때문에, export되지 않은 함수나 변수에 액세스하여 테스트할 <strong>수</strong> 있다.
또한 이후에는 공개된 API만을 테스트하는 방법에 대해서도 알아볼 것이다.</p> <p>먼저, 간단한 함수를 작성해 보자.</p> <pre class="language-go"><!></pre> <p>또한, 이에 대응하는 테스트 코드도 작성해준다.</p> <pre class="language-go"><!></pre> <p>모든 테스트는 <em>_test.go</em>로 끝나는 이름을 가진 파일에 작성된다.
만약 <em>foo.go</em> 파일에 대한 테스트를 하고자 한다면, 테스트는 <em>foo_test.go</em>라는 파일에 작성되어야 할 것이다.</p> <p>테스트 함수는 <code>Test</code>로 시작하며, <code>*testing.T</code> 타입의 단일 파라미터를 받는다. 보통 이 파라미터의 이름은 <code>t</code>로 한다. 함수는 아무 것도 반환하지 않는다.
테스트의 이름은 그 대상을 문서화하기 위한 것이기 때문에, 테스트할 대상을 잘 설명할 수 있도록 지어주어야 한다.
함수에 대한 unit test를 작성할 때는 <code>Test</code> 다음에 함수 이름을 넣는 것이 일반적이다.
만약 export되지 않은 함수에 대한 테스트라면 <code>Test</code>와 함수 이름 사이에 언더바(_)를 넣기도 한다.</p> <p>또한, 표준 Go 코드를 사용하여 테스트 중인 코드를 호출하고 응답이 예상대로 이루어지는지 확인한다.
만약 결과가 옳지 않다면, <code>fmt.Print()</code>와 유사하게 동작하는 <code>t.Error()</code> 메소드를 통해 에러 보고를 만든다. 다른 에러 보고 메소드에 대해서도 차차 살펴볼 것이다.</p> <p>바이너리를 빌드할 때 <code>go build</code>, 실행할 때 <code>go run</code> 명령어를 치듯, 테스트 할 때는 <code>go test</code>를 치면 된다.</p> <pre class="language-text"><!></pre> <p>이렇듯 테스트가 성공하지 못했음을 확인하였다. 우리가 작성한 함수에서 버그를 찾아 고쳐주자. <code>return x + x</code>를 <code>return x + y</code>로 고쳐주고 다시 <code>go test</code>를 실행하면 테스트가 성공적으로 끝남을 확인할 수 있다.</p> <pre class="language-go"><!></pre> <p><code>go test</code> 명령어는 어떤 패키지에서 테스트를 진행할 지 명시할 수 있다. <code>./...</code>를 붙이면 현재 디렉토리와 모든 하위 디렉토리에 대해서 테스트를 진행한다. <code>-v</code> 플래그가 있으면 verbose output을 출력한다.</p> <br/><br/> <h3 id="reporting-test-failures"><a aria-hidden="true" tabindex="-1" href="#reporting-test-failures"><span class="icon icon-link"></span></a>Reporting Test Failures</h3> <p><code>*testing.T</code>에는 테스트 실패를 보고하기 위한 몇 개의 메소드가 있다. <code>fmt.Print()</code>와 유사한 <code>Error()</code>가 있듯, 포매팅을 할 수 있는 <code>fmt.Printf()</code>와 유사한 형태의 <code>Errorf()</code>가 존재한다.</p> <pre class="language-go"><!></pre> <p><code>Error()</code>나 <code>Errorf()</code>는 테스트가 실패했음을 명시하기 위해 사용되지만, 테스트되는 함수의 코루틴은 계속해서 실행된다.
만약 테스트가 실패하면 그 이상의 처리를 멈춰야 한다고 생각한다면 <code>Fatal()</code> 및 <code>Fatalf()</code> 메소드를 사용하면 된다. <code>Fatal()</code>은 <code>Error()</code>와, <code>Fatalf()</code>는 <code>Errorf()</code>와 유사하다.
이들의 차이점은 테스트 실패 메시지가 생성되자마자 테스트 함수가 즉시 종료된다는 점이다.
다만 모든 테스트가 종료되는 것이 아니라, 현재 진행중인 테스트만 종료되는 것이다. 다른 나머지 테스트는 이후에 계속 진행된다.</p> <p>언제 <code>Error()</code>/<code>Errorf()</code>를, 그리고 언제 <code>Fatal()</code>/<code>Fatalf()</code>를 써야 할까?
만일 테스트 실패로 인해 동일한 테스트 함수의 이후 테스트가 항상 실패하거나, 테스트가 panic 상태에 빠지는 경우 <code>Fatal()</code>/<code>Fatalf()</code>를 사용한다.
만약 여러 개의 서로 다른 항목에 대해 테스트하는 경우 <code>Error()</code>/<code>Errorf()</code>를 사용해야 최대한 많은 문제를 발견하여 테스트를 여러 번 돌릴 일 없게 만들어준다.</p> <br/><br/> <h3 id="setting-up-and-tearing-down"><a aria-hidden="true" tabindex="-1" href="#setting-up-and-tearing-down"><span class="icon icon-link"></span></a>Setting Up and Tearing Down</h3> <p>테스트를 실행하기 전에 설정하고, 테스트가 완료되면 제거할 수 있는 특정 상태가 있을 수 있다.
이 경우 <code>TestMain</code>함수를 사용하여 이러한 상태를 관리한다.</p> <pre class="language-go"><!></pre> <p><code>*testing.M</code> 타입의 파라미터를 받는 함수 <code>TestMain()</code>을 선언하였고, 패키지 레벨 변수 <code>testTime</code>을 참조하는 <code>TestFirst()</code>, <code>TestSecond()</code> 함수가 있다. <code>TestMain()</code> 함수가 있는 패키지에서 <code>go test</code>를 실행하면 테스트를 직접 호출하는 대신 <code>TestMain()</code> 함수가 호출된다.
일단 상태가 설정되면, <code>*testing.M</code>의 <code>Run()</code> 메소드를 호출한다. <code>Run()</code> 메소드는 exit code를 반환하며, 0은 모든 테스트에 통과했음을 나타낸다.
마지막으로 이 exit code와 함께 <code>os.Exit()</code>를 호출하면 된다.</p> <p><code>go test</code>를 실행한 결과는 다음과 같다.</p> <pre class="language-text"><!></pre> <p><code>TestMain()</code>은 딱 한 번만 호출되며, 패키지당 하나의 <code>TestMain()</code>만 사용할 수 있다. <code>TestMain()</code>을 유용하게 사용할 만한 상황은 다음과 같다.</p> <ul><li>데이터베이스 등, 외부 repository를 구축해야 하는 경우</li> <li>테스트하는 코드가 초기화해야 하는 패키지 레벨 변수에 의존하는 경우</li></ul> <p>이전에 언급하였듯 애초에 패키지 레벨 변수의 사용은 웬만하면 자제해야 한다. 프로그램에서 데이터가 어떻게 흘러가는지 이해하기 어렵게 만들기 때문이다.
따라서 <code>TestMain()</code>을 사용해야 하는 경우, 그보다 코드를 리팩토링하는 것을 먼저 고려해보는 것이 낫다.</p> <p><code>*testing.T</code>의 <code>Cleanup()</code> 메소드는 하나의 테스트를 위해 생성된 임시 리소스를 청소하는 데 사용한다. <code>Cleanup()</code> 메소드는 반환값 및 파라미터가 없는 함수 하나를 파라미터로 받는데, 이 함수는 테스트가 완료되면 실행된다.
간단한 테스트라면 <code>defer</code>문을 사용하더라도 동일한 결과를 얻을 수 있을것이다.
하지만 아래와 같이 샘플 데이터를 설정할 때 helper function에 의존하는 경우 <code>Cleanup()</code>이 유용하다. <code>defer</code>처럼 <code>Cleanup()</code>을 여러 번 호출하여도 괜찮으며, 마지막에 추가된 함수가 먼저 호출된다.</p> <pre class="language-go"><!></pre> <br/><br/> <h3 id="storing-sample-test-data"><a aria-hidden="true" tabindex="-1" href="#storing-sample-test-data"><span class="icon icon-link"></span></a>Storing Sample Test Data</h3> <p><code>go test</code> 명령어는 현재의 패키지 디렉토리를 작업 디렉토리로 설정한다.
만약 특정 패키지에서 함수를 테스트하고자 할 때 샘플 데이터를 사용하고 싶다면, <em>testdata</em>라는 서브디렉토리를 만들면 된다. 이 디렉토리명은 Go에서 테스트 파일을 저장하는 공간으로 지정되어 있다. <em>testdata</em>에서 데이터를 읽어올 때는 반드시 상대경로를 사용한다. <code>go test</code> 명령어는 작업 디렉토리를 현재 패키지로 변경하기 때문에, 각각의 패키지는 상대 경로를 통해 <em>testdata</em>에 접근할 것이다.</p> <p>예제 코드는 <a href="https://github.com/learning-go-book/test_examples/tree/master/text" rel="nofollow">여기</a>에서 찾아볼 수 있다.</p> <br/><br/> <h3 id="testing-your-public-api"><a aria-hidden="true" tabindex="-1" href="#testing-your-public-api"><span class="icon icon-link"></span></a>Testing Your Public API</h3> <p>우리가 작성한 테스트 코드는 프로덕션 코드와 동일한 패키지에 위치하였다.
이 같은 방식으로 export된 함수나 export되지 않은 함수까지도 테스트할 수 있었다.</p> <p>만약 패키지의 Public API만 테스트하려는 거라면, 이를 위한 Go의 규칙이 마련되어 있다.
프로덕션 코드와 같은 디렉토리에 테스트 코드를 두는 것은 동일하지만, 패키지 명으로 <code>[패키지명]_test</code>를 사용한다.</p> <p>맨 처음 작성하였던 <code>adder</code>의 예제를 떠올려보자. 맨 처음의 <code>adder</code>는 export되지 않는 함수였다. 이번에는 export되게끔 아래와 같이 작성한다. 파일명은 <em>adder_public.go</em>로 하였다.</p> <pre class="language-go"><!></pre> <p>이렇게 export된 Public API에 대해, 테스트 코드를 아래와 같이 작성해준다.
파일명 또한 기존 테스트 코드처럼 뒤에 <em>_test</em>를 붙여, <em>adder_public_test.go</em>로 하였다.</p> <pre class="language-go"><!></pre> <p>주목할 점은 테스트 코드의 패키지명이 <code>adder_test</code>라는 것이다. 패키지가 다르기 때문에 <code>AddNumbers()</code>를 <code>adder</code>로부터 import해와야 하며, <code>adder.AddNumbers()</code>로 호출한다.
또한 테스트의 이름 설정 규칙에 따라, 테스트 함수의 이름은 <code>AddNumbers()</code> 함수와 매칭되어야 한다.</p> <p>패키지 안에서 export된 함수를 호출할 수 있는 것처럼, 소스 코드와 동일한 패키지에서 Public API를 테스트할 수 있다.
위와 같은 <code>_test</code> 접미사 패키지를 사용하면 export된 함수, 메소드, 타입, 상수, 변수에만 접근이 가능해지기 때문에, 블랙 박스 테스팅이 강제된다.
또한 <code>_test</code>가 붙은 패키지와 붙지 않은 두 개의 패키지가 동일한 디렉토리에 있을 수 있다.</p> <br/><br/> <h3 id="use-go-cmp-to-compare-test-results"><a aria-hidden="true" tabindex="-1" href="#use-go-cmp-to-compare-test-results"><span class="icon icon-link"></span></a>Use go-cmp to Compare Test Results</h3> <p>복합 유형 인스턴스끼리 정확히 비교하는 코드는 다소 길어질 수 있다. <code>reflect.DeepEqual()</code>을 사용하여 struct, map, slice를 비교할 수 있지만 더 나은 방법이 있다.
구글은 <code>go-cmp</code>라는 서드파티 모듈을 출시했다. <code>go-cmp</code>는 비교를 하여 일치하지 않는부분에 대한 상세한 설명을 제공한다.
간단한 <code>struct</code>와 함께 <code>go-cmp</code>를 사용해보자.</p> <pre class="language-go"><!></pre> <p>먼저 구조체 타입과 이에 대한 Factory 함수를 정의해주었다.
이제 테스트 코드를 작성해보자.</p> <pre class="language-go"><!></pre> <p>먼저, 테스트 코드를 실행하려면 <code>github.com/google/go-cmp/cmp</code>를 임포트해주어야 한다. <code>cmp.Diff()</code> 함수는 예상되는 출력값과 테스트하는 함수의 실제 출력값을 파라미터로 받는다.
그리고 두 입력 간의 불일치가 있다면 이에 대한 설명을 출력한다. 만약 입력이 일치한다면, 빈 문자열을 반환한다.</p> <p><code>go test</code>를 입력하여, <code>go-cmp</code>가 어떤 결과를 내놓았을 지 확인해보자.</p> <pre class="language-text"><!></pre> <p><code>+</code>와 <code>-</code>가 있는 라인에서 예측값과 실제 값의 차이를 나타낸다.
위 예제의 경우 <code>DateAdded</code> 필드가 일치하지 않았기 때문에 실패하였다. <code>DateAdded</code> 필드는 <code>testcmp.CreatePerson()</code>를 통해서만 제대로 처리할 수 있기 때문에, <code>DateAdded</code> 필드는 검사 대상에서 제외할 필요가 있다.
아래와 같이 비교 함수를 정의해서, 비교하고자 하는 필드를 지정할수 있다.</p> <pre class="language-go"><!></pre> <p><code>cmp.Comparer()</code>에 두 인스턴스를 비교하는 익명 함수를 전달하여 비교 함수를 생성하였다.
...
위 예제에서는 <code>Name</code> 및 <code>Age</code> 필드만 비교하여 <code>DateAdded</code> 필드는 생략하였다.</p> <p>이후 <code>cmp.Diff()</code>를 호출할 때 비교 함수를 마지막 파라미터로 함께 보내주면 된다.</p> <p><code>go-cmp</code>에 대한 간략한 사용법을 리뷰하였다. 추가적인 사용 예는 <a href="https://pkg.go.dev/github.com/google/go-cmp@v0.5.9/cmp" rel="nofollow">문서</a>를 참고해보자!</p> <br/><br/> <h2 id="table-tests"><a aria-hidden="true" tabindex="-1" href="#table-tests"><span class="icon icon-link"></span></a>Table Tests</h2> <hr/> <p>일반적으로 함수가 올바르게 동작하는 지 확인하기 위해서는 하나 이상의 테스트케이스가 필요한 법이다.
...
이런 코드를 반복하여 작성하는 것보단 <em>table test</em>라 불리는 패턴을 사용하는 것이 좋을 수 있다.</p> <p>가령, <code>table</code> 패키지에서 아래와 같은 함수를 테스트한다고 가정해보자.</p> <pre class="language-go"><!></pre> <p>이런 함수를 테스트하려면 각각의 경우에 대해 입력을 집어넣어서 유효한 결과를 성공적으로 반환하는지, 또는 에러를 잘 반환하는지 등 확인해볼 필요가 있다.
테스트 코드를 아래와 같이 작성할 수도 있지만, 그야말로 쌩노가다이다..!</p> <pre class="language-go"><!></pre> <p>이러한 반복되는 코드를 table test로 교체해보자.
먼저, 익명 구조체의 slice를 선언한다. 이 구조체는 테스트명, 입력 파라미터, 반환값을 포함한다.
slice의 각 항목은 각각의 테스트 케이스를 의미한다.</p> <pre class="language-go"><!></pre> <p>이제 <code>data</code>에서 반복문을 돌며, 각 테스트 케이스별로 <code>*testing.T</code>의 <code>Run()</code> 메소드를 수행할 것이다. <code>Run()</code>은 테스트 케이스의 이름과 <code>*testing.T</code> 단일 파라미터를 가진 함수를 파라미터로 받는다.
이 함수 안에서는 <code>DoMath()</code>를 호출하여 <code>data</code>의 각 필드값을 사용하여 반복적으로 테스트를 진행한다.
또한 테스트 명령어 입력시 <code>-v</code> 플래그를 주면 각 테스트케이스가 테스트명과 함께 출력된다.</p> <pre class="language-text"><!></pre> <blockquote><p>위 예제에서는 <code>error</code>의 에러 메시지를 비교하여 예측 에러와 실제 에러를 비교하지만, 실제로는 <code>errors.Is()</code> 또는 <code>errors.As()</code>를 사용하여 에러를 검증하는 편이 더 이상적이다.</p></blockquote> <br/><br/> <h2 id="code-coverage"><a aria-hidden="true" tabindex="-1" href="#code-coverage"><span class="icon icon-link"></span></a>Code Coverage</h2> <p>Code Coverage는 놓친 테스트 케이스를 찾아낼 수 있는 유용한 도구이다. 다만 100%의 Code Coverage라고 해서 코드가 어떠한 입력에 대해 버그를 일으키지 않으리라는 것을 의미하지는 않는다.</p> <pre class="language-text"><!></pre> <p>위와 같이 <code>go test</code> 명령어에 <code>-cover</code>플래그를 달면 Code Coverage를 계산할 수 있다.
또한 <code>-coverprofile</code> 플래그로 출력 파일명을 명시하면 coverage에 대한 정보를 파일로 저장할 수 있다.</p> <pre class="language-text"><!></pre> <p>이처럼 coverage의 달성률을 보여주며, 파일은 아래와 같이 저장된다.</p> <pre class="language-text"><!></pre> <p>마지막이 0으로 끝나는 라인이, 테스트 케이스에 의해 cover되지 않은 코드의 위치를 의미한다.</p> <br/> <p>이 방식도 충분히 좋지만, 조금 더 직관적인 도구가 <code>cover</code>라는 도구이다. HTML로 coverage 정보를 소스코드와 함께 나타내주기 때문에 매우 직관적이다.
아래 명령어를 통해 실행해보자.</p> <pre class="language-text"><!></pre> <p>명령어를 입력하면 브라우저가 열리고, 아래 사진처럼 보다 직관적으로 붉게 색칠된 영역의 코드를 통해 테스트 케이스에 의해 cover되지 않은 코드를 보여준다!</p> <!> <p>단순한 인터페이스이다. 좌측 상단 콤보박스에 테스트된 파일의 목록이 나타나며, 회색/빨상색/초록색으로 코드가 cover되는지 여부를 나타낸다.</p> <p>우리는 잘못된 연산자 기호를 입력 파라미터로 넘기는 테스트케이스를 작성하지 않았기 때문에, <code>default</code> case가 cover되지 않았다. 만약 테스트 <strong>케이스</strong> slice에 다음과 같은 항목을 추가한다면 문제가 해결될 것이다.</p> <pre class="language-go"><!></pre> <p>추가하였다면 아래 명령어를 입력하여 재실행해보자.</p> <pre class="language-text"><!></pre> <p>이전에 cover되지 않았던 <code>default</code> case가 통과한 것을 알 수 있을 것이다. 귀찮으니 사진은 생략해야지~</p> <blockquote><p>다시 한번 강조하지만, 100%의 Code Coverage라고 해서 코드에 버그가 없는 것이 아니니 Code Coverage만 철썩같이 믿는 일은 없도록 해야 한다..!</p></blockquote> <br/><br/> <h2 id="benchmarks"><a aria-hidden="true" tabindex="-1" href="#benchmarks"><span class="icon icon-link"></span></a>Benchmarks</h2> <p>작성된 코드가 얼마나 빠른지/느린지, 즉 성능을 판단하는 것은 매우 어려운 일이다.
이를 우리가 스스로 판단하기 보단 Go에서 제공하는 도구를 사용하는 것이 훨씬 좋을 것이다.</p> <p>아래의 예제 함수를 보자.</p> <pre class="language-go"><!></pre> <p>파일 내 문자의 수를 세는 간단한 함수로, 파일명과 파일로부터 읽어올 버퍼의 크기를 파라미터로 받는다.
당연히 버퍼의 크기에 따라 이터레이션을 도는 횟수가 달라질 테니, 버퍼 크기에 따라 성능 차이가 많이 날 것이다.</p> <p>먼저, 함수가 문제 없이 잘 동작하는지 확인해보도록 하자</p> <pre class="language-go"><!></pre> <p>랜덤 시드가 고정값이라, 아마 테스트에 통과하긴 할 것이다.
그리고 우리의 목표는, 최적의 버퍼 크기를 찾아내는 것이다.</p> <blockquote><p>최적화를 하기 위해 긴 여정을 떠나기에 앞서, 진짜 최적화를 할 필요가 있는 것인지에 대해 검토하는 게 좋다.
가령, 프로그램이 이미 응답 속도나 메모리 요구사항을 충족하고 있다면 기능 추가나 버그 수정에 시간을 쓰는 게 낫다.
비즈니스 요구사항에 따라 응답 속도 및 메모리 요구사항이 정의된다.</p></blockquote> <br/> <p>Go에서 벤치마크 함수를 작성하려면, 테스트 코드에서 <code>Benchmark</code>로 시작하는 함수 이름을 지어주면 된다.
또한 <code>*testing.B</code> 타입의 단일 파라미터를 받으며, <code>*testing.B</code>는 벤치마킹 관련 기능과 함께 <code>*testing.T</code>의 모든 기능을 포함한다.</p> <p>먼저, 버퍼 크기가 1 바이트인 경우의 벤치마크를 측정해보자.</p> <pre class="language-go"><!></pre> <p>위 코드에서 패키지 레벨 변수인 <code>blackhole</code>을 사용하는 것이 다소 의아해 보인다.
책에서는 <code>FileLen()</code>을 반복 호출하는 과정에서 컴파일러가 알아서 최적화하는 것을 막기 위함이라고 하는 모양이다.</p> <p>아무튼, Go의 벤치마크는 반드시 0부터 <code>b.N</code>만큼의 이터레이션을 돌아야 한다.
테스트 프레임워크는 타이밍 결과가 정확할 때까지 N 값을 늘려가며 벤치마크 함수를 돌릴 것이다.
출력된 결과에 대해서는 조금 있다 확인할 것이다.</p> <p>벤치마크를 돌리기 위해서는 <code>go test</code> 명령어에 <code>-bench</code> 플래그를 추가해주면 된다.
이 플래그는 실행하고자 하는 벤치마크의 이름을 표현하는 정규식을 받는다. <code>-bench=.</code>와 같이 주면 모든 벤치마크를 실행한다.
한편 <code>-benchmem</code> 플래그를 추가하면 벤치마크 출력에 메모리 할당 관련 정보를 포함한다.
또한 벤치마크 이전에 테스트가 실행되므로, 테스트가 통과될 때만 코드를 벤치마크할 수 있다.</p> <p>다음은 <code>-benchmem</code> 플래그를 활성화하여 명령어를 입력한 뒤, 출력된 결과를 확인해보자.</p> <pre class="language-text"><!></pre> <p>각 정보는 다음과 같은 의미를 갖는다.</p> <ul><li><code>BenchmarkFileLen1-12</code>: 벤치마크명-벤치마크의 GOMAXPROCS 값</li> <li><code>66</code>: 안정적인 결과를 얻기 위해 실행한 테스트의 수</li> <li><code>17917232 ns/op</code>: 하나의 벤치마크를 통과하는 데 걸린 시간(나노초)</li> <li><code>129 B/op</code>: 하나의 벤치마크를 통과하는데 할당된 바이트 수</li> <li><code>4 allocs/op</code>: 하나의 벤치마크를 통과하는 동안 힙에서 바이트를 할당한 횟수 (할당된 바이트 수보다 작거나 같음)</li></ul> <br/> <p>이제 1 바이트 말고, 버퍼 크기를 키워서 벤치마크를 돌려보자.</p> <pre class="language-go"><!></pre> <p><code>t.Run()</code>을 통해 table test를 하던 것처럼, <code>b.Run()</code>으로 여러 입력의 벤치마크를 실행할 수 있다.
다음과 같은 결과를 얻었다.</p> <pre class="language-text"><!></pre> <p>어느 정도 충분히 예상할 수 있는 결과였다.
...
또한, 성능을 높이려면 메모리를 많이 써야 하고, 메모리를 적게 쓰려면 성능을 포기해야 하는 trade-off에 대해서도 확인할 수 있다.</p> <blockquote><p>벤치마킹을 통해 성능 및 메모리 문제를 발견했다면, 다음 단계는 문제가 정확히 무엇인지 파악하는 것이다.
Go에는 프로그램의 CPU 및 메모리 사용량 데이터를 수집하는 프로파일링 및 시각화 도구가 포함되어 있다.
이에 대한 내용은 <a href="https://oreil.ly/HHe9c" rel="nofollow">여기</a>를 참고하면 좋을 것 같다.</p></blockquote> <br/><br/> <h2 id="stubs"><a aria-hidden="true" tabindex="-1" href="#stubs"><span class="icon icon-link"></span></a>Stubs</h2> <p>지금까지 우리가 작성한 테스트는 다른 코드에 의존하지 않는 함수에 대한 테스트였다. 하지만 대부분의 코드는 의존성으로 가득하다!</p> <p>Go에서는 함수 호출을 추상화하는 두 가지 방법이 있는데, 바로 함수 타입을 정의하는 것과 인터페이스를 정의하는 것이다.
이러한 추상화는 프로덕션 코드를 모듈화함으로써 유닛 테스트를 수월하게 할 수 있게끔 해준다.</p> <p>아래의 예제를 살펴보자.</p> <pre class="language-go"><!></pre> <p><code>Processor</code>라는 구조체 타입과 <code>MathSolver</code>라는 인터페이스 타입을 작성하였다. <code>MathSolver</code>의 구현체는 추후 작성할 것이다.</p> <p><code>Processor</code>는 <code>io.Reader</code>로부터 표현식을 읽어 계산된 값을 반환하는 메소드 <code>ProcessExpression()</code>를 가지고 있다.</p> <pre class="language-go"><!></pre> <p><code>ProcessExpression()</code>을 테스트하기 위한 코드를 작성해보자.
먼저, 필요한 것은 <code>Reslove()</code> 메소드를 충족시키는 간단한 Stub을 정의하는 것이다.</p> <pre class="language-go"><!></pre> <p>다음으로, 이 Stub을 사용하는 유닛 테스트를 작성하는 것이다. 실제로는 에러 메시지에 대한 검증까지도 해야 하지만, 테스트의 간소화를 위해 생략하였다.</p> <pre class="language-go"><!></pre> <p>이렇게 작성한 코드에 대해 테스트를 돌려 보면, 잘 동작함을 확인할 수 있다.</p> <br/> <p>대부분의 Go의 인터페이스는 1개 내지는 2개 정도의 메소드만을 명시하지만, 가끔은 그 이상의 많은 메소드를 가진 인터페이스를 작성하게 될 떄도 있을 것이다.
가령, 아래와 같은 인터페이스가 있다고 해보자.</p> <pre class="language-go"><!></pre> <p>이렇게 거대한 인터페이스의 테스트 코드를 작성하는 방법은 크게 두 가지 패턴이 있다.</p> <p>하나는 구조체에 인터페이스를 임베드하는 것이다.
인터페이스를 구조체에 임베딩하면 자동으로 인터페이스의 모든 메소드가 구조체에서 정의된다.
다만 메소드에 대한 구현이 되는 것까진 아니기 때문에, 현재 테스트에서 사용할 메소드는 구현해주어야 한다.</p> <pre class="language-go"><!></pre> <p>위와 같이, <code>Entities</code>를 필드로 갖는 구조체 <code>Logic</code>과 테스트하고자 하는 메소드인 <code>GetPetNames()</code>를 작성하였다.
이 메소드에서는 <code>Entities</code>의 <code>GetPets()</code> 메소드 하나밖에 사용하지 않는다.
따라서 <code>Entities</code>의 모든 메소드에 대한 Stub을 만들어주기 보단, <code>GetPets()</code>의 Stub만 만들어주는 게 편할 것이다.</p> <p>아래처럼, <code>Entities</code>를 임베딩하는 구조체를 만들고, <code>GetPets()</code>의 Stub을 만들어보자.</p> <pre class="language-go"><!></pre> <p><code>GetPetNamesStub</code>에서 <code>GetPets()</code>를 제외한 다른 메소드들은 구현되지 않긴 하지만 정의는 되어 있으므로 <code>Entities</code>를 충족시킨다.
따라서 위처럼 <code>GetPets()</code>만 작성해도 된다.</p> <p>이제 <code>GetPetNames()</code>의 유닛 테스트를 작성할 수 있다. 테스트 코드를 작성해보자!</p> <pre class="language-go"><!></pre> <p>이대로 <code>go test</code>를 실행하면 아마 <code>case1</code>, <code>case2</code>에서 테스트를 통과하지 못할 텐데, <code>GetPetNames()</code>에 작은 버그가 있어서 그렇다. 에러를 해결하고 나면 <code>case1</code>, <code>case2</code>를 통과하는 것을 볼 수 있을 것이다.</p> <p>이처럼 인터페이스를 구조체에 임베드하여 사용하는 경우, 구현하지 않은 메소드를 호출해선 안된다. 아마 테스트 중 panic이 발생할 것이다! 따라서 인터페이스의 메소드 중, 테스트하는 함수에서 사용되는 것은 반드시 구현해줘야 한다.</p> <br/> <p>인터페이스에서 1~2개 정도의 메소드를 구현하는 것은 단일 테스트에서는 별 문제가 없다.
하지만 입/출력이 다른 여러 테스트에서 동일한 메소드를 호출해야 할 때 문제가 발생할 수 있다.
이런 경우, 모든 테스트에 대해 가능한 모든 결과를 구현체 내에 포함하거나, 각 테스트에 대해 구현체를 다시 작성해야 한다.
이러한 구현체는 이해하거나 유지보수하기 매우 어려울 것이다.</p> <p>더 좋은 방식은 함수 필드를 통해 우회하여 메소를 호출하는 Stub 구조체를 만드는 것이다. <code>Entities</code>에 정의된 각 메소드와 일치하게끔, Stub 구조체의 함수 필드를 정의해준다.</p> <pre class="language-go"><!></pre> <p><code>EntitiesStub</code>에 함수 필드를 정의하고, <code>Entities</code> 인터페이스를 충족시키게끔 메소드를 정의해준다.
각각의 메소드에서는 대응되는 함수 필드를 호출한다.</p> <pre class="language-go"><!></pre> <p>위 예제의 Table test에서는 ‘data’ 구조체에 함수 필드를 추가해주었으며, getPets를 호출했을 때 반환할 데이터를 이 함수 필드에 명시할 수 있다.
이와 같이 Stub을 작성하는 경우, 각 테스트 케이스에서 Stub이 반환해야 하는 값이 확실하다는 장점이 있다.
각 테스트마다 새로운 <code>EntitiesStub</code> 인스턴스가 생성되고, <code>EntitiesStub</code>의 함수 필드인 <code>getPets()</code>에는 <code>data</code>에 명시된 함수 필드가 할당되므로, 직접 작성한 테스트 데이터로 <code>GetPetNames()</code>를 테스트할 수 있다.</p> <blockquote><p>Mock과 Stub은 자주 혼용되는 단어이지만 실제로는 서로 다른 개념이다.
요약하자면 Stub은 주어진 입력에 대한 출력값을 검사하는 것이라면, Mock은 주어진 입력에 대해 일련의 호출이 예상된 순서로 발생하는지 검사하는 것이다.</p> <p>본 장에서 Stub 코드를 작성하였듯, Mock 관련 코드도 작성할 수 있다.
가장 유명한 Mock 괸련 라이브러리는 구글의 <a href="https://github.com/golang/mock" rel="nofollow">gomock</a>과 Stretchr사의 <a href="https://github.com/stretchr/testify" rel="nofollow">testify</a>이다.
후자가 github star 수는 더 높다!</p></blockquote> <br/><br/> <h2 id="httptest"><a aria-hidden="true" tabindex="-1" href="#httptest"><span class="icon icon-link"></span></a>httptest</h2> <p>HTTP 서비스를 호출하는 함수의 테스트 코드를 작성하는 것은 쉽지 않다.
보통 이런 경우 Integration test가 되며, 함수가 호출하는 서비스의 테스트 인스턴스를 준비해야 했다.
하지만 Go는 표준 라이브러리에 <code>net/http/httptest</code> 패키지를 포함하고 있으며, 덕분에 HTTP 서비스의 stub을 작성하기가 굉장히 쉽다.</p> <p><code>solver</code> 패키지로 되돌아가서, 표현식을 평가하기 위해 HTTP 서비스를 호출하는 MathSolver의 구현체를 작성해보자.</p> <pre class="language-go"><!></pre> <p>이제, <code>httptest</code> 라이브러리를 사용하여 서버를 구축하지 않고 이 코드를 테스트할 것이다.
먼저, 함수로 전달된 데이터가 서버에 도착하는지 확인해야 한다.
따라서 테스트 함수 안에서 입력과 출력을 저장할 <code>info</code>라고 하는 타입을 선언하고, 현재 입출력이 할당될 <code>io</code>라는 변수를 정의한다.</p> <pre class="language-go"><!></pre> <p>다음으로, 가짜 원격 서버를 구축하고 이를 사용하여 <code>RemoteSolver</code> 인스턴스를 구성한다.</p> <pre class="language-go"><!></pre> <p><code>httptest.NewServer()</code> 함수는 사용되지 않는 무작위 포트에 http 서버를 열며, 요청을 처리하기 위한 <code>http.Handler</code> 구현체를 설정해주어야 한다.
이것은 서버이기 때문에 테스트가 끝나면 닫아줘야 한다. 위 예제에서는 <code>defer</code>를 통해 close한다. 그리고 <code>URL</code>과 <code>http.Client</code>를 <code>RemoteSolver</code>에 전달한다.</p> <p>이렇게 셋업하고 나면, 테스트 함수의 나머지 부분은 다른 Table test와 크게 다를 바 없다.</p> <pre class="language-go"><!></pre> <p>위 코드에서 변수 <code>io</code>는 Stub 서버용과 테스트 함수 양쪽에서 Closure에 의해 캡처된다.
테스트 코드 쪽에서 write, Stub 쪽에서 read가 일어나는데, 이와 같은 방식은 이러한 테스트 코드라면 몰라도 프로덕션 코드에서는 지양해야 한다.</p> <br/><br/> <h2 id="integration-test"><a aria-hidden="true" tabindex="-1" href="#integration-test"><span class="icon icon-link"></span></a>Integration Test</h2> <p><code>httptest</code> 등을 쓰면 외부 서비스에 대한 테스트를 생략할 수 있지만, Integration Test를 작성해야 할 때가 올 것이며, 이를 통해 서비스 API에 대한 이해가 올바른지 확인할 수 있을 것이다.</p> <p>문제는 자동화된 테스트를 그룹화하는 방법을 찾는 것이다.
적랍한 환경이 있는 경우에만 Integration Test를 실행해야 할 것이며, 일반적으로 Integration Test는 Unit Test보다 느리기 때문에 자주 실행하진 않을 것이다.</p> <p>Go의 컴파일러는 코드를 컴파일할 때 <em>build tag</em>를 사용할 수 있게 해준다.
빌드 태그는 파일의 첫 번째 줄에 <code>// +build</code>로 시작하는 주석을 달아서 지정할 수 있다.
...
반면 빌드 태그가 있는 테스트는 필요한 리소스가 있을 때만 실행된다.</p> <p>solver 패키지로 돌아와서, Integration Test를 작성할 것이다.
그 이전에 다음과 같은 Docker 명령어를 쳐서 서버 이미지를 다운받고, 8080 포트로 로컬 서버를 열어보자.</p> <pre class="language-bash"><!></pre> <p>이제 Resolve 메소드가 서버와 잘 통신하는지 테스트하는 Integration Test를 작성해볼 것이다.</p> <pre class="language-go"><!></pre> <p>파일의 맨 처음 부분은 <code>// +build integration</code>로 시작하며, 다른 부분은 이전에 작성하였던 table test와 크게 다르지 않다!</p> <p>이렇게, 빌드 태그를 지정한 테스트를 실행하려면 다음과 같이 명령어를 실행하면 된다.</p> <pre class="language-bash"><!></pre> <p><code>-v</code> 플래그로 인해 어떤 테스트가 실행되었나 확인할 수 있을 것이다.
앞서 설명하였듯, 테스트 태그를 지정하지 않은 테스트들도 <code>integration</code> 태그의 테스트와 함께 모두 실행된 것을 확인할 수 있다.</p> <blockquote><p><code>go test</code>에는 <code>-short</code> 플래그를 통해 테스트 시간이 오래 소요되는 항목을 생략할 수 있는 기능이 있다.
다만 short flag를 사용하면 안된다는 목소리도 있고, 좀 의견이 분분한 모양이다.</p> <p>short flag에 대해 알고 싶다면 <a href="https://learning.oreilly.com/library/view/learning-go/9781492077206/ch13.html#:-:text=Using%20the%20-short%20Flag" rel="nofollow">여기</a>를 참고하자.</p></blockquote> <br/><br/> <h2 id="race-checker"><a aria-hidden="true" tabindex="-1" href="#race-checker"><span class="icon icon-link"></span></a>Race Checker</h2> <p>동시성 문제를 해결하기 위해 Go에서는 여러 가지 지원을 해주지만, 버그는 여전히 발생할 수 있다.
이를테면 서로 다른 고루틴이 lock 없이 하나의 변수에 접근하는 일은 충분히 발생할 수 있는 일이다.
이러한 상황을 <em>data race</em>라고 한다.</p> <p>Go에서는 <em>Race Checker</em>를 통해 이러한 버그를 찾아내는 데 도움을 받을 수 있다.
코드에서 모든 data race를 찾아낸다고 보장하지는 않지만, data race를 찾을 경우 찾은 데이터에 적절한 lock을 설정해줘야 한다.</p> <p>다음의 예제를 살펴보자.</p> <pre class="language-go"><!></pre> <p>만약 race condition을 배제한다면, <code>getCounter()</code>의 반환값은 5000일 것이다. 하지만 race condition때문에 현실은 그렇지가 않다. 테스트 함수를 작성해보자.</p> <pre class="language-go"><!></pre> <p>간단한 테스트 함수이다. 출력된 결과가 예상값인 5000인지 아닌지만 테스트해주고 있다. <code>go test</code>를 입력하여 테스트를 수행하면,</p> <pre class="language-text"><!></pre> <p>테스트를 통과할 때도 있지만, 가끔 이렇게 결과가 5000이 아닐 때가 있다!</p> <p>공유된 변수 <code>counter</code>에 접근하여 값을 증가시키는 과정에서, data race로 인해 변수의 값 갱신이 누락되는 현상이 생기는 것이다.
아주 간단한 예제를 소개했지만, 실제로 코드에서는 data race를 유발하는 원인을 훨씬 찾기 힘들 것이다.
그만큼 <em>Race Checker</em>가 상당히 유용하게 사용될 수 있다!</p> <p><em>Race Checker</em>를 사용하려면 <code>go test</code> 명령어에 <code>-race</code> 플래그를 붙이면 된다.</p> <pre class="language-text"><!></pre> <p>이처럼 race condition을 유발하는 코드 라인, 고루틴이 생성된 라인 등을 추적해준다.</p> <br/> <p>프로그램을 빌드할 때도 <code>-race</code> 플래그를 붙여서 race checker를 실행할 수 있다. 이렇게 하면 빌드 이전에 테스트를 거치지 않고도 data race를 추적해볼 수 있다. 다만 <code>-race</code> 플래그가 붙은 채로 빌드된 바이너리는 속도가 매우 느리기 때문에, 프로덕션 코드에서는 절대 권장되지 않는다.</p> <br/><br/> <h2 id="references"><a aria-hidden="true" tabindex="-1" href="#references"><span class="icon icon-link"></span></a>References</h2> <hr/> <center><p>[</p> <!> ](https://learning.oreilly.com/library/view/learning-go/9781492077206/) <br/> [Jon Bodner, 『Learning Go』, O'Reilly Media, Inc.](https://learning.oreilly.com/library/view/learning-go/9781492077206/)</center> <br/><br/>`,1);function Ss(en){var sn=is(),p=n(es(sn),28),cn=s(p);t(cn,()=>`<code class="language-go"><span class="token keyword">package</span> adder
...
<span class="token punctuation">&#125;</span></code>`),a(p);var o=n(p,4),un=s(o);t(un,()=>`<code class="language-go"><span class="token keyword">package</span> adder
...
<span class="token punctuation">&#125;</span></code>`),a(o);var e=n(o,10),ln=s(e);t(ln,()=>`<code class="language-text">$ go test
--- FAIL: Test_addNumbers (0.00s)
    adder_test.go:8: incorrect result: expected 5, got 4
FAIL
exit status 1
FAIL    github.com/jhseoeo/learning-go/13/adder     0.001s</code>`),a(e);var c=n(e,4),kn=s(c);t(kn,()=>`<code class="language-go">$ <span class="token keyword">go</span> test
PASS
ok      github<span class="token punctuation">.</span>com<span class="token operator">/</span>jhseoeo<span class="token operator">/</span>learning<span class="token operator">-</span><span class="token keyword">go</span><span class="token operator">/</span><span class="token number">13</span><span class="token operator">/</span>adder     <span class="token number">0</span><span class="token punctuation">.</span>001s</code>`),a(c);var u=n(c,11),rn=s(u);t(rn,()=>'<code class="language-go">t<span class="token punctuation">.</span><span class="token function">Errorf</span><span class="token punctuation">(</span><span class="token string">"incorrect result: expected %d, but got %d"</span><span class="token punctuation">,</span> <span class="token number">5</span><span class="token punctuation">,</span> result<span class="token punctuation">)</span></code>'),a(u);var l=n(u,13),dn=s(l);t(dn,()=>`<code class="language-go"><span class="token keyword">package</span> testmain
...
<span class="token punctuation">&#125;</span></code>`),a(l);var i=n(l,6),gn=s(i);t(gn,()=>`<code class="language-text">$ go test
...
ok      github.com/jhseoeo/learning-go/13/testmain  0.001s</code>`),a(i);var k=n(i,10),bn=s(k);t(bn,()=>`<code class="language-go"><span class="token keyword">package</span> cleanup
...
<span class="token punctuation">&#125;</span></code>`),a(k);var r=n(k,22),fn=s(r);t(fn,()=>`<code class="language-go"><span class="token keyword">package</span> adder

<span class="token keyword">func</span> <span class="token function">AddNumbers</span><span class="token punctuation">(</span>x<span class="token punctuation">,</span> y <span class="token builtin">int</span><span class="token punctuation">)</span> <span class="token builtin">int</span> <span class="token punctuation">&#123;</span>
	<span class="token keyword">return</span> x <span class="token operator">+</span> x
<span class="token punctuation">&#125;</span></code>`),a(r);var d=n(r,4),mn=s(d);t(mn,()=>`<code class="language-go"><span class="token keyword">package</span> adder_test
...
<span class="token keyword">func</span> <span class="token function">TestAddNumbers</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">if</span> result <span class="token operator">!=</span> <span class="token number">5</span> <span class="token punctuation">&#123;</span>
		t<span class="token punctuation">.</span><span class="token function">Error</span><span class="token punctuation">(</span><span class="token string">"incorrect result: expected 5, got"</span><span class="token punctuation">,</span> result<span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(d);var g=n(d,13),yn=s(g);t(yn,()=>`<code class="language-go"><span class="token keyword">type</span> Person <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
...
<span class="token keyword">func</span> <span class="token function">CreatePerson</span><span class="token punctuation">(</span>name <span class="token builtin">string</span><span class="token punctuation">,</span> age <span class="token builtin">int</span><span class="token punctuation">)</span> Person <span class="token punctuation">&#123;</span>
    ...
    <span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(g);var b=n(g,4),hn=s(b);t(hn,()=>`<code class="language-go"><span class="token keyword">package</span> cmp_test
...
<span class="token keyword">func</span> <span class="token function">TestCreatePerson</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(b);var f=n(b,6),wn=s(f);t(wn,()=>`<code class="language-text">$ go test
...
FAIL
exit status 1
FAIL    github.com/jhseoeo/learning-go/13-tests/cmp 0.001s</code>`),a(f);var m=n(f,4),vn=s(m);t(vn,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestCreatePerson_IgnoreDate</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	expected <span class="token operator">:=</span> testcmp<span class="token punctuation">.</span>Person<span class="token punctuation">&#123;</span>
		Name<span class="token punctuation">:</span> <span class="token string">"Dennis"</span><span class="token punctuation">,</span>
		Age<span class="token punctuation">:</span>  <span class="token number">37</span><span class="token punctuation">,</span>
	<span class="token punctuation">&#125;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(m);var y=n(m,19),xn=s(y);t(xn,()=>`<code class="language-go"><span class="token keyword">package</span> table
...
<span class="token keyword">func</span> <span class="token function">DoMath</span><span class="token punctuation">(</span>num1<span class="token punctuation">,</span> num2 <span class="token builtin">int</span><span class="token punctuation">,</span> op <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">int</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(y);var h=n(y,4),_n=s(h);t(_n,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestDoMath</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(h);var w=n(h,4),Tn=s(w);t(Tn,()=>`<code class="language-go">data <span class="token operator">:=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
		name     <span class="token builtin">string</span>
	...
	<span class="token punctuation">&#125;</span></code>`),a(w);var v=n(w,4),Pn=s(v);t(Pn,()=>`<code class="language-text">$ go test -v
=== RUN   TestDoMath
...
PASS
ok      github.com/jhseoeo/learning-go/13-tests/table       0.001s</code>`),a(v);var x=n(v,11),Sn=s(x);t(Sn,()=>'<code class="language-text">go test -v -cover -coverprofile=c.out</code>'),a(x);var _=n(x,4),En=s(_);t(En,()=>`<code class="language-text">$ go test -v -cover -coverprofile=c.out

...
coverage: 87.5% of statements</code>`),a(_);var T=n(_,4),Gn=s(T);t(Gn,()=>`<code class="language-text">mode: set
...
github.com/jhseoeo/learning-go/13-tests/table/table.go:14.16,16.4 1 1</code>`),a(T);var P=n(T,8),Mn=s(P);t(Mn,()=>'<code class="language-text">go tool cover -html=c.out</code>'),a(P);var an=n(P,4);on(an,{alt:"go cover screenshot",src:"/post_img/Go/Golang_basics/GO13/2023-02-08-01-35-22.png"});var S=n(an,6),Cn=s(S);t(Cn,()=>'<code class="language-go"><span class="token punctuation">&#123;</span><span class="token string">"bad_op"</span><span class="token punctuation">,</span> <span class="token number">2</span><span class="token punctuation">,</span> <span class="token number">2</span><span class="token punctuation">,</span> <span class="token string">"?"</span><span class="token punctuation">,</span> <span class="token number">0</span><span class="token punctuation">,</span> <span class="token string">&#96;unknown operator ?&#96;</span><span class="token punctuation">&#125;</span><span class="token punctuation">,</span></code>'),a(S);var E=n(S,4),Nn=s(E);t(Nn,()=>'<code class="language-text">go test -v -cover -coverprofile=c.out &amp;&amp; go tool cover -html=c.out</code>'),a(E);var G=n(E,15),Rn=s(G);t(Rn,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">FileLen</span><span class="token punctuation">(</span>f <span class="token builtin">string</span><span class="token punctuation">,</span> bufsize <span class="token builtin">int</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">int</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(G);var M=n(G,6),Fn=s(M);t(Fn,()=>`<code class="language-go"><span class="token keyword">package</span> bench_test
...
<span class="token keyword">func</span> <span class="token function">TestFileLen</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(M);var C=n(M,12),In=s(C);t(In,()=>`<code class="language-go"><span class="token keyword">var</span> blackhole <span class="token builtin">int</span>
...
<span class="token keyword">func</span> <span class="token function">BenchmarkFileLen1</span><span class="token punctuation">(</span>b <span class="token operator">*</span>testing<span class="token punctuation">.</span>B<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(C);var N=n(C,10),An=s(N);t(An,()=>'<code class="language-text">BenchmarkFileLen1-12	66	17917232 ns/op	129 B/op 	4 allocs/op</code>'),a(N);var R=n(N,10),Ln=s(R);t(Ln,()=>`<code class="language-go"><span class="token keyword">var</span> blackhole <span class="token builtin">int</span>
...
<span class="token keyword">func</span> <span class="token function">BenchmarkFileLen2</span><span class="token punctuation">(</span>b <span class="token operator">*</span>testing<span class="token punctuation">.</span>B<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	<span class="token keyword">for</span> <span class="token boolean">_</span><span class="token punctuation">,</span> v <span class="token operator">:=</span> <span class="token keyword">range</span> <span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token builtin">int</span><span class="token punctuation">&#123;</span><span class="token number">1</span><span class="token punctuation">,</span> <span class="token number">10</span><span class="token punctuation">,</span> <span class="token number">100</span><span class="token punctuation">,</span> <span class="token number">1000</span><span class="token punctuation">,</span> <span class="token number">10000</span><span class="token punctuation">,</span> <span class="token number">1000000</span><span class="token punctuation">&#125;</span> <span class="token punctuation">&#123;</span>
		...
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(R);var F=n(R,4),Dn=s(F);t(Dn,()=>`<code class="language-text">BenchmarkFileLen2/FileLen-1-12			66		17921247 ns/op	129 B/op		4 allocs/op
...
BenchmarkFileLen2/FileLen-1000000-12	24865	47952 ns/op		1007753 B/op	4 allocs/op</code>`),a(F);var I=n(F,17),Bn=s(I);t(Bn,()=>`<code class="language-go"><span class="token keyword">type</span> Processor <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
	Solver MathSolver
<span class="token punctuation">&#125;</span>
...
<span class="token keyword">func</span> <span class="token function">readToNewLine</span><span class="token punctuation">(</span>r io<span class="token punctuation">.</span>Reader<span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">string</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">return</span> <span class="token function">string</span><span class="token punctuation">(</span>out<span class="token punctuation">)</span><span class="token punctuation">,</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),a(A);var L=n(A,4),jn=s(L);t(jn,()=>`<code class="language-go"><span class="token keyword">type</span> MathSolverStub <span class="token keyword">struct</span><span class="token punctuation">&#123;</span><span class="token punctuation">&#125;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(L);var D=n(L,4),qn=s(D);t(qn,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestProcessorProcessExpression</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
( 2 + 2 * 10&#96;</span><span class="token punctuation">)</span>
	...
	<span class="token keyword">for</span> i<span class="token punctuation">,</span> d <span class="token operator">:=</span> <span class="token keyword">range</span> data <span class="token punctuation">&#123;</span>
		...
		<span class="token punctuation">&#125;</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(D);var B=n(D,8),Wn=s(B);t(Wn,()=>`<code class="language-go"><span class="token keyword">type</span> Entities <span class="token keyword">interface</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(B);var U=n(B,6),Hn=s(U);t(Hn,()=>`<code class="language-go"><span class="token keyword">type</span> Logic <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
	Entities Entities
<span class="token punctuation">&#125;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(U);var j=n(U,6),On=s(j);t(On,()=>`<code class="language-go"><span class="token keyword">type</span> GetPetNamesStub <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
	Entities
<span class="token punctuation">&#125;</span>
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>ps GetPetNamesStub<span class="token punctuation">)</span> <span class="token function">GetPets</span><span class="token punctuation">(</span>userId <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token punctuation">[</span><span class="token punctuation">]</span>Pet<span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(j);var q=n(j,6),$n=s(q);t($n,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestLogicGetPetNames</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">for</span> <span class="token boolean">_</span><span class="token punctuation">,</span> d <span class="token operator">:=</span> <span class="token keyword">range</span> data <span class="token punctuation">&#123;</span>
		...
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(q);var W=n(q,12),Kn=s(W);t(Kn,()=>`<code class="language-go"><span class="token keyword">type</span> EntitiesStub <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(W);var H=n(W,4),zn=s(H);t(zn,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestLogicGetPetNames2</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	data <span class="token operator">:=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
		name     <span class="token builtin">string</span>
	...
	<span class="token keyword">for</span> <span class="token boolean">_</span><span class="token punctuation">,</span> d <span class="token operator">:=</span> <span class="token keyword">range</span> data <span class="token punctuation">&#123;</span>
		...
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(H);var O=n(H,15),Qn=s(O);t(Qn,()=>`<code class="language-go"><span class="token keyword">type</span> RemoteSolver <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(O);var $=n(O,4),Vn=s($);t(Vn,()=>`<code class="language-go"><span class="token keyword">type</span> info <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span>
<span class="token keyword">var</span> io info</code>`),a($);var K=n($,4),Zn=s(K);t(Zn,()=>`<code class="language-go">server <span class="token operator">:=</span> httptest<span class="token punctuation">.</span><span class="token function">NewServer</span><span class="token punctuation">(</span>
...
<span class="token punctuation">&#125;</span></code>`),a(K);var z=n(K,6),Jn=s(z);t(Jn,()=>`<code class="language-go">data <span class="token operator">:=</span> <span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
	name   <span class="token builtin">string</span>
	io     info
...
<span class="token keyword">for</span> <span class="token boolean">_</span><span class="token punctuation">,</span> d <span class="token operator">:=</span> <span class="token keyword">range</span> data <span class="token punctuation">&#123;</span>
	t<span class="token punctuation">.</span><span class="token function">Run</span><span class="token punctuation">(</span>d<span class="token punctuation">.</span>name<span class="token punctuation">,</span> <span class="token keyword">func</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		...
		<span class="token punctuation">&#125;</span>
	<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),a(z);var Q=n(z,17),Xn=s(Q);t(Xn,()=>`<code class="language-bash"><span class="token function">docker</span> pull jonbodner/math-server
<span class="token function">docker</span> run <span class="token parameter variable">-p</span> <span class="token number">8080</span>:8080 jonbodner/math-server</code>`),a(Q);var V=n(Q,4),Yn=s(V);t(Yn,()=>`<code class="language-go"><span class="token comment">// +build integration</span>
...
<span class="token keyword">func</span> <span class="token function">TestRemoteSolver_ResolveIntegration</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span>
</code>`),a(V);var Z=n(V,6),ns=s(Z);t(ns,()=>'<code class="language-bash">go <span class="token builtin class-name">test</span> <span class="token parameter variable">-tags</span> integration</code>'),a(Z);var J=n(Z,17),ss=s(J);t(ss,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">getCounter</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token builtin">int</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(J);var X=n(J,4),as=s(X);t(as,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">TestGetCounter</span><span class="token punctuation">(</span>t <span class="token operator">*</span>testing<span class="token punctuation">.</span>T<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(X);var Y=n(X,4),ts=s(Y);t(ts,()=>`<code class="language-text">--- FAIL: TestGetCounter (0.00s)
    race_test.go:8: unexpected counter: 4116
FAIL
exit status 1
FAIL    github.com/jhseoeo/learning-go/13-tests/race        0.001s</code>`),a(Y);var nn=n(Y,8),ps=s(nn);t(ps,()=>`<code class="language-text">$ go test -race
==================
WARNING: DATA RACE
...
FAIL
exit status 1
FAIL    github.com/jhseoeo/learning-go/13-tests/race        0.007s</code>`),a(nn);var tn=n(nn,15),os=n(s(tn),2);on(os,{alt:"Learning Go Book Cover",src:"https://learning.oreilly.com/covers/urn:orm:book:9781492077206/400w/"}),pn(3),a(tn),pn(3),us(en,sn)}export{Ss as default,ls as metadata};
