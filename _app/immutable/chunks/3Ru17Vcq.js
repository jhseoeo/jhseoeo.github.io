import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{s as n,f as an,c as s,r as a,n as S}from"./CVx5jffJ.js";import{f as tn,a as pn}from"./2cXyNWGb.js";import{h as t}from"./D6IIVJDJ.js";import{I as on}from"./BVXqo7x9.js";const en={title:"컨텍스트",date:"2022-09-09T00:00:00.000Z",excerpt:"The Context",categories:["Golang","Basic Golang"],coverImage:"/post_img/Go/Golang_basics/cover.png",coverWidth:16,coverHeight:9,indexed:!1,exposed:!0},{title:fn,date:xn,excerpt:hn,categories:wn,coverImage:yn,coverWidth:bn,coverHeight:mn,indexed:vn,exposed:Cn}=en;var cn=tn(`<br/> <p>본 글은 Golang을 공부하며 주요 내용이라 생각되는 것들을 기록해둔 자료이며, Ubuntu 22.04 LTS 기준으로 작성되었습니다.</p> <br/><br/> <h2 id="introduction"><a aria-hidden="true" tabindex="-1" href="#introduction"><span class="icon icon-link"></span></a>Introduction</h2> <hr/> <p>서버는 각각의 요청에 대해 메타데이터를 핸들링해야 할 필요가 있다.
이 메타데이터는 넓게 보면 두 종류로 분류할 수 있다.
바로 요청을 올바르게 처리하기 위해 필요한 메타데이터와, 요청 처리를 언제 중지해야 할지를 나타내는 메타데이터다.</p> <p>예를 들면 어느 HTTP 서버에서는 추적 ID를 사용하여, 마이크로서비스를 통해 request chain을 식별하려고 할 수 있다.
또한 시간이 너무 오래 걸릴 수도 있으니, 다른 마이크로서비스의 종료를 요청하는 타이머를 설정할 수도 있다.</p> <p>대부분의 언어는 <em>threadlocal</em> 변수를 사용하여 이러한 정보들을 저장하고, 특정한 쓰레드에 데이터를 연결한다.
다만 Go의 고루틴은 값을 조회하는데 사용할 수 있는 고유한 ID가 없기 때문에, Go에서는 할 수 없는 방식이다.</p> <p>Go는 <strong>context</strong>라는 구성을 통해 이러한 요청 메타데이터 문제를 해결한다. 컨텍스트를 사용하는 방법을 살펴보도록 하자.</p> <br/><br/> <h2 id="what-is-the-context"><a aria-hidden="true" tabindex="-1" href="#what-is-the-context"><span class="icon icon-link"></span></a>What Is the Context?</h2> <hr/> <p>컨텍스트는 <code>context</code> 패키지에 정의된 <code>Context</code> 인터페이스를 충족시키는 인스턴스일 뿐이다. 달리 추가적인 기능이 아니다.
...
보통 컨텍스트 파라미터의 변수명은 대개 <code>ctx</code>로 한다.</p> <pre class="language-go"><!></pre> <br/> <p><code>context</code> 패키지는 <code>Context</code> 인터페이스를 정의하는것 외에도,
컨텍스트를 만들고 래핑하기 위한 몇 가지 factory function을 포함한다.</p> <p>CLI 프로그램의 시작 지점처럼 기존 context가 없는 경우, <code>context.Background()</code> 함수를 통해 비어 있는 컨텍스트를 생성한다.
일반적으로 함수에서 concrete type을 반환하는 것과는 달리, 이 함수는 <code>context.Context</code> 타입을 반환한다.</p> <p>비어 있는 컨텍스트는 시작점이라 할 수 있다.
컨텍스트에 메타데이터를 추가할 때마다, <code>context</code> 패키지의 factory function 중 하나를 사용하여 기존 컨텍스트를 <em>감싸주면</em> 된다.</p> <pre class="language-go"><!></pre> <br/> <blockquote><p><code>context.TODO()</code>도 비어 있는 <code>context.Context()</code> 인스턴스를 만드는 함수로, 개발 도중 임시적으로 필요한 경우 사용하게끔 의도되었다.
컨텍스트를 어디서 가져와서 어떻게 사용할지 확실하지 않은 경우, <code>context.TODO()</code>를 사용하면 좋다.
다만 릴리즈할 코드에서는 <code>context.TODO()</code>를 포함하면 안된다.</p></blockquote> <br/> <p>HTTP 서버를 작성할 때는, 컨텍스트를 획득하고 미들웨어 레이어를 거쳐 <code>http.Handler</code>까지 전달하는 데 다소 다른 패턴을 사용한다.
컨텍스트는 <code>net/http</code> 패키지가 생긴 지 한참 뒤에 생긴 개념이기에,
호환성 문제로 인해 <code>http.Handler</code> 인터페이스에 <code>context.Context</code>를 파라미터로 추가하지 못했다.</p> <p>대신 기존 타입에 새로운 메소드를 추가하였다. <code>http.Request</code>에는 컨텍스트에 관련된 두 개의 메소드가 존재한다.</p> <ul><li><code>Context()</code> 메소드는 해당 요청과 관련된 <code>context.Context</code> 인스턴스를 리턴한다.</li> <li><code>WithContext()</code> 메소드는 <code>context.Context</code> 인스턴스를 파라미터로 받아서,
해당 컨텍스트와 기존의 상태가 결합된 새로운 <code>http.Request</code> 인스턴스를 반환한다.</li></ul> <br/> <p>아래 코드는 일반적인 패턴이다.</p> <pre class="language-go"><!></pre> <p>미들웨어에서 첫 번째로 하는 일은 <code>Context()</code> 메소드를 사용해, 요청으로부터 기존 컨텍스트를 추출해내는 것이다.
컨텍스트에 값을 추가하고 나면, <code>WithContext()</code> 메소드를 사용해 기존 요청과 값이 추가된 컨텍스트를 기반으로 새로운 컨텍스트를 만들어낸다.
마지막으로 <code>handler</code>를 호출하여 기존의 <code>http.ResponseWriter</code>와 새로운 <code>*http.Request</code>를 보낸다.</p> <br/> <p>handler도 마찬가지로, <code>Context</code> 메소드를 사용하여 요청으로부터 컨텍스트를 추출하고,
앞서 본 것처럼 컨텍스트를 첫 번째 파라미터로 보내면서 비즈니스 로직을 호출한다.</p> <pre class="language-go"><!></pre> <br/> <p><code>WithContext()</code> 메소드를 사용하는 경우가 한 가지 더 있다.
바로 어플리케이션에서 다른 HTTP 서비스로 HTTP 호출을 하는 경우이다.
컨텍스트를 미들웨어로 보낼 때처럼, <code>WithContext()</code>를 사용해 나가는 요청의 컨텍스트를 설정해야 한다.</p> <pre class="language-go"><!></pre> <p>자동완성에 뜨길래 본건데, <code>http.NewRequest()</code>를 이렇게 쓸 바에는 <code>http.NewRequestWithContext()</code>를 쓰면 일반적인 패턴대로 컨텍스트를 첫 번째 파라미터로 넘길 수 있는 것 같다.</p> <p>아무튼 이제 컨텍스트를 획득하고 보내는 방법을 알게 되었으니, Cancellation을 통해 더 유용하게 사용하는 방법을 알아보자.</p> <br/><br/> <h2 id="cancellation"><a aria-hidden="true" tabindex="-1" href="#cancellation"><span class="icon icon-link"></span></a>Cancellation</h2> <hr/> <p>여러 개의 고루틴을 생성하여, 서로 다른 HTTP 서비스를 호출하는 요청이 있다고 가정해보자.
만약 서비스 하나에서 에러가 발생하여 더 이상 유효한 값을 얻을 수 없다고 한다면, 다른 고루틴을 계속 돌릴 이유가 없어진다.
이러한 맥락에서 <strong>Cancellation</strong>을 사용하며, 컨텍스트에서 이를 사용하는 방법에 대해 알아볼 것이다.</p> <p>cancellable한 컨텍스트를 만들기 위해서는 <code>context.WithCancel()</code> 함수를 사용한다.
...
또한 child 컨텍스트는 parent 컨텍스트를 감싼(wrap)다. <code>context.CancelFunc()</code> 함수는 말 그대로 컨텍스트를 취소(cancel)하며, 취소 여부를 확인하고 있는 모든 코드에 처리를 중단하라는 신호를 보낸다.</p> <p>기본적으로 컨텍스트는 immutable한 인스턴스로 처리되기 때문에,
정보를 추가하기 위해 컨텍스트 안의 값을 변경하기보단 이를 감싸는 child 컨텍스트를 생성한다.
이런 방식을 통해, 컨텍스트를 코드의 가장 깊은 레이어까지 전달할 수 있다.
반대로, 코드의 깊은 레이어에서 표면적인 레이어쪽으로 정보를 전달하는데 사용되지는 않는다.</p> <br/> <p>실제로 동작하는 예제를 살펴보자.
먼저, <code>go mod init [MODULE_NAME]</code>으로 모듈을 선언해준다.
그리고 <em>servers.go</em> 파일을 생성하여 아래와 같이 작성한다.</p> <pre class="language-go"><!></pre> <p><code>httptest.Server</code>는 원격 서버와 통신하는 코드에 대해 unit test를 하기에 보다 용이하게 설계된 타입이다.
특히 동일한 프로그램 안에 서버와 클라이언트가 존재할 경우 매우 유용하다.</p> <p>위 예제에는 서로 다른 각각의 서버를 여는 두 개의 함수가 정의되었다.
하나는 요청이 오면 2초동안 기다렸다가 메시지를 리턴하는 서버이며,
하나는 <code>error</code>라는 쿼리스트링이 <code>true</code>로 설정되어 있다면 <code>error</code>라는 메시지를, 아니면 <code>ok</code>라는 메시지를 보낸다.</p> <br/> <p>이제 코드의 클라이언트 부분인, <em>client.go</em> 파일을 작성해보자.</p> <pre class="language-go"><!></pre> <p>위 예제의 <code>callBoth()</code> 함수에서는 <code>context.WithCancel()</code>에 파라미터로 넘어온 컨텍스트를 넘김으로써, cancellable한 컨텍스트와 취소 함수를 생성하며,
이 취소 함수의 이름은 <code>cancel</code>로 짓는 게 국룰이라고 한다.</p> <p>기억해야 할 점 중 하나는 cancellable한 컨텍스트를 생성하면 취소 함수를 <em>반드시</em> 호출해주어야 한다는 것이다.
만약 그렇게 하지 않으면 고루틴 또는 메모리에서 리소스 낭비가 발생할 것이다.
취소 함수는 한 번 호출하면 두 번째부터의 호출은 무시되므로, 여러번 호출해도 상관없다.
위 예제에서는 <code>defer</code>를 사용하여 결과적으로 반드시 호출되게 해주었다.</p> <p>또한 cancellable한 컨텍스트, 라벨, URL 등을 캡쳐하여 두 개의 고루틴을 설정 및 실행해주었고, <code>sync.WaitGroup</code>의 <code>sync()</code> 메소드를 사용하여 고루틴들이 끝날 때까지 대기해주었다.</p> <p><code>callServer()</code> 함수는 간단한 클라이언트로, 취소 가능한 컨텍스트를 통해 요청을 생성하여 전송한다.
만약 에러가 발생하거나 서버로부터 받은 문자열 결과값이 <code>error</code>라면, 에러를 리턴한다.</p> <br/> <p>이제 메인함수를 살펴보자. <em>main.go</em> 파일을 아래와 같이 작성한다.</p> <pre class="language-go"><!></pre> <p><code>main()</code> 함수에서는 서버를 열고, 생성된 컨텍스트로 클라이언트를 돌려 서버에 요청을 보낸다.</p> <br/> <p>작성된 코드를 빌드하여 실행해보자.</p> <pre class="language-bash"><!></pre> <p>위 명령어를 치면 모듈이 빌드되어 실행 파일이 생성된다. 나의 경우 생성된 실행 파일은 <em>context-cancel</em>이었다.
두 가지 경우에 대해, 해당 파일을 실행해보자. 아래 경우는 에러가 발생하지 않았을 때이다.</p> <pre class="language-bash"><!></pre> <br/> <p>아래 경우는 에러가 발생하였을 때이다.</p> <pre class="language-bash"><!></pre> <p><code>fastServer</code>에 대한 요청에서 에러가 발생하였으므로 컨텍스트가 취소된다.
이 때 <code>NewRequestWithContext()</code>가 실행 중이므로, 컨텍스트의 취소를 감지하여, 고루틴이 멈추게 된 것이다.</p> <blockquote><p>만약 위 프로그램을 실행해보았다면, 에러가 발생했음에도 프로그램이 즉각 종료되지 않았다는 점을 알 수 있을 것이다.
이는 <code>slowServer()</code>가 종료되는 것을 기다리기 때문이다.
만약 <code>slowServer()</code>의 타임아웃을 2초에서 6초로 늘린다면, <code>httptest.Server blocked in Close after 5 seconds, waiting for connections</code>와 같이 신기한 에러 메세지가 출력되는 것을 확인할 수 있을 것이다.</p> <p><code>slowServer()</code>를 아래와 재작성해주면 컨텍스트의 취소를 제대로 처리하여, 즉시 서버가 종료될 것이다.</p> <pre class="language-go"><!></pre> <p>아마 클라이언트 단의 컨텍스트가 취소되면, 요청에 있는 컨텍스트의 <code>Done</code> 신호가 들어오는 것 같다.</p></blockquote> <p>위 예제들의 소스코드는 <a href="https://github.com/jhseoeo/Learning-golang/tree/master/12-context/cancellation" rel="nofollow">여기</a>에서 확인해볼 수 있다.</p> <br/> <p>이렇게, 수동으로 컨텍스트를 취소하는 방법에 대해 살펴보았다.
이러한 방식도 유용하지만, 다음 섹선에서는 타임아웃을 통해 취소를 자동화하는 방법에 대해 살펴볼 것이다.</p> <br/><br/> <h2 id="timers"><a aria-hidden="true" tabindex="-1" href="#timers"><span class="icon icon-link"></span></a>Timers</h2> <hr/> <p>서버가 하는 일 중 가장 중요한 것 중 하나는 요청들을 관리하는 것이다.
아기 개발자들은 가끔 서버가 가능한 한 많은 요청을 받고, 각 클라이언트에 결과를 반환할 수 있을 때까지 작업을 해야 한다고 생각한다.
하지만 이는 아기 개발자들의 잘못된 생각이다. 응애!</p> <p>서버는 공유 리소스와 비슷한 성격을 가진다.
각 사용자들은 서버에 가능한 많은 요청을 보내고 결과를 얻고 싶어하며, 이 때 다른 사용자의 요청에 대해서는 별 관심이 없다.
자고로 공유 리소스는 모든 사용자에게 동일한 시간만큼 제공할 수 있도록 관리할 필요성이 있다.</p> <br/> <p>서버는 로드(작업량)를 관리하기 위해, 보통 네 가지 방법을 사용한다.</p> <ol><li>동시에 처리하는 요청의 수 제한</li> <li>대기 큐에서 기다리고 있는 요청의 수 제한</li> <li>요청이 처리되는 시간 제한</li> <li>요청이 사용할 수 있는 리소스의 양 제한(메모리, 디스크 등)</li></ol> <p>Go에서는 앞의 세 개의 제한을 처리하는 도구를 제공한다.
특히 1번과 2번의 경우 이미 우리가 본 것이다.
1번의 경우 고루틴의 수를 제한하면 서버가 동시에 처리하는 요청의 수가 관리되는 것이며,
2번은 buffered channel의 크기를 조절하면 해결된다.</p> <p>그리고, 컨텍스트를 통해 요청이 얼마나 오래동안 처리될 수 있는지를 제한할 수 있다.
어플리케이션을 만들 때, 사용자가 불만족스러운 경험을 하기 전에 요청을 완료하려면 얼마나 시간이 걸릴 지에 대해 알고 있어야 한다.
만약 요청을 실행할 수 있는 최대 시간을 알고 있다면, 요청에 컨텍스트를 적용할 수 있다.</p> <br/> <p>시간이 제한된 컨텍스트를 생성하는 방법은 두 가지가 있다.
하나는 <code>context.WithTimeout()</code> 함수를 사용하는 것이다. 이 함수는 기존의 컨텍스트와 타임아웃을 나타내는 <code>time.Duration</code> 인스턴스를 파라미터로 받는다.
그리고 명시된 만큼의 시간이 지나면 자동으로 취소되는 컨텍스트와, 컨텍스트를 바로 취소할 수 있는 취소 함수가 함께 리턴된다.</p> <p>두 번째 방법은 <code>context.WithDeadline()</code> 함수를 사용하는 것이다.
이 함수도 마찬가지로 기존의 컨텍스트를 받으며, <code>time.Duration</code> 인스턴스 대신 <code>time.Time</code> 인스턴스를 파라미터로 받는다. <code>context.WithTimeout()</code> 함수처럼 해당 시각이 지나면 컨텍스트는 자동으로 취소되는 컨텍스트와 컨텍스트를 바로 취소할 수 있는 취소 함수가 리턴된다.
만약 <code>context.WithDeadline()</code>에 과거의 <code>time.Time</code> 시각을 보내면, 이미 취소된 컨텍스트를 리턴한다.</p> <p>만약 컨텍스트가 자동으로 닫히는 시각을 알아내고 싶다면, <code>context.Context</code> 인스턴스의 <code>Deadline()</code> 메소드를 사용한다.
해당 메소드는 컨텍스트가 닫히는 시각을 나타내는 <code>time.Time</code> 인스턴스와, 타임아웃이 설정되어 있는지 여부를 <code>true</code>나 <code>false</code>로 리턴한다.
따라서 <em>comma ok idiom</em>을 적용하면 된다.</p> <p>적용된 예제가 길어져서, <a href="https://github.com/jhseoeo/Learning-golang/tree/master/12-context/context_timeout.go" rel="nofollow">여기</a>에서 확인하면 될 듯 하다.</p> <br/> <p>요청에 대한 전반적인 타임아웃을 설정할 때, 이 타임아웃을 세분화할 수도 있다.
...
각각에 호출에 소요되는 시간을 제어할 수 있는 것이다.</p> <p>특히 child 컨텍스트의 타임아웃은 parent 컨텍스트의 타임아웃에 종속된다.
이를테면 parent 컨텍스트의 타임아웃이 2초이고 child 컨텍스트의 타임아웃이 3초라고 하면,
parent 컨텍스트의 타임아웃인 2초가 지나면 child 컨텍스트도 자동으로 취소된다.</p> <p>마찬가지로 예제가 좀 길어져서, <a href="https://github.com/jhseoeo/Learning-golang/tree/master/12-context/parent_child_timeout.go" rel="nofollow">여기</a>서 확인하면 된다.
이 코드를 실행해보면 parent 컨텍스트의 타임아웃이 먼저 찾아올 것이고, 그와 동시에 child 컨텍스트도 취소된다.</p> <br/><br/> <h2 id="handling-context-cancellation-in-your-own-code"><a aria-hidden="true" tabindex="-1" href="#handling-context-cancellation-in-your-own-code"><span class="icon icon-link"></span></a>Handling Context Cancellation in Your Own Code</h2> <hr/> <p>대부분의 경우 코드 내에서 타임아웃이나 취소에 대해 걱절할 필요는 없다.
일반적인 코드들은 기란이 그렇게 오래 걸리지 않는다.
다만 다른 HTTP 서비스를 호출하거나, 데이터베이스를 호출할 때에는 컨텍스트를 파라미터로 잘 넘겨주어야 한다.
얘네들은 보통 타임아웃이 발생하거나 오류가 발생하여 취소를 할만한 상황이 종종 나오는 편이므로, 컨텍스트를 넘김으로써 취소를 처리해주기 용이하다.</p> <p>만약 컨텍스트가 취소되면 중단되어야 하는 코드를 작성하는 경우, 동시성을 통해 취소 체크를 할 수 있다(10장에서 이미 살펴보았다). <code>context.Context</code> 타입은 취소를 관리하는 두 가지 메소드를 포함하고 있다. <code>Done()</code> 메소드는 <code>struct&#123;&#125;</code> 타입의 채널을 리턴한다. 해당 채널은 취소 함수나 타임아웃이 발생할 때 close된다.
이 때 closed channel에 읽기를 시도하면 반드시 zero value를 리턴한다.</p> <blockquote><p>이 때 취소할 수 없는 컨텍스트에서 <code>Done()</code> 메소드를 호출하면 <code>nil</code>을 반환한다.
게다가 <code>nil</code> 채널에서 읽기를 시도하면 프로그램이 끝날 때까지 읽기를 시도할 것이기 때문에 멈출 것이다.</p></blockquote> <br/> <p><code>context.Context</code>의 <code>Err()</code> 메소드는 만약 컨텍스트가 아직 활성화되어 있다면 <code>nil</code>을 반환한다.
반면 만약 컨텍스트가 취소되어 있다면, <code>context.Cancelled</code> 또는 <code>context.DeadlineExceeded</code> 둘 중 하나의 sentinel error를 반환한다. <code>context.Cancelled</code>는 컨텍스트를 명시적으로 취소했을 때, <code>context.DeadlineExceeded</code>는 타임아웃이 발생했을 때 리턴된다.</p> <pre class="language-go"><!></pre> <p><code>longRunningThing()</code> 함수는 타임아웃이 발생할 때까지 <code>result</code>의 값을 누적하는 함수이다.
(원래 타임아웃 자체가 에러 상황을 상정하기 때문에, 실제로 이런 함수를 작성하면 안된다!) <code>case &lt;-ctx.Done():</code>로 타임아웃이 발생할 시 함수가 리턴된다.</p> <p><code>longRunningThingManager()</code> 함수는 <code>select</code>문을 통해 <code>longRunningThing()</code> 함수의 값이 반환되기까지 기다리거나,
혹은 컨텍스트가 취소될 때까지 기다린다. 취소된다면 에러와 함께 함수가 리턴된다.</p> <p>이 때 각각의 <code>select</code>문의 <code>case &lt;-ctx.Done():</code> 안에, <code>ctx.Err()</code>가 있음을 확인할 수 있다.
이 메소드가 어떤 출력 결과를 반환하는지에 집중하여 확인해볼 것이다.</p> <br/> <p>이어서 <code>main()</code> 함수를 작성해보자.</p> <pre class="language-go"><!></pre> <p><code>time.AfterFunc(time.Second*3, cancel)</code> 라인의 유무에 따라 출력이 달라지는 것을 확인할 수 있다.
이 라인을 지운다면 컨텍스트가 타임아웃에 의해 취소되며, 지우지 않는다면 컨텍스트가 <code>cancel</code> 함수에 의헤 취소되기 때문이다.</p> <br/> <p>본 절에서는 컨텍스트를 통한 타임아웃의 명시 및 취소 방법에 대해 알아보았다.
물론 이전에 보았던 <code>time.After()</code> 함수를 통해서도 취소가 가능하지만, 개인적으로는 컨텍스트를 통한 방식이 더 좋아보이는 것 같다.</p> <br/><br/> <h2 id="values"><a aria-hidden="true" tabindex="-1" href="#values"><span class="icon icon-link"></span></a>Values</h2> <hr/> <p>컨텏트의 또 다른 용도라고 할 수 있다. 컨텍스트는 프로그램의 다른 컴포넌트에 요청에 대한 메타데이터를 전달하는 방법을 제공한다.</p> <p>물론 파라미터를 통해 명시적으로 데이터를 전달하는 방식을 최우선적으로 고려하는 것이 좋다.
Go에서는 명시적인 데이터 전달을 이상적으로 여긴다.
특히 함수가 특정 데이터를 반드시 필요로 한다면, 해당 데이터가 어디서 오는지(보내는지) 명확하게 정의되어야 한다.</p> <p>하지만 데이터를 명시적으로 전달하지 못할 만한 상황이 존재할 수밖에 없다.
그런 경우는 대부분 HTTP 요청 핸들러와 관련 미들웨어에서 발생한다.</p> <p>앞서 보았듯 모든 요청 핸들러는 각각 request, response를 파라미터로 받는다.
그런데 미들웨어의 핸들러가 해당 값을 사용할 수 있게 만들려면, context에 값을 저장해주어야 한다.
이러한 경우는 유저의 JWT(JSON Web Token)을 추출하거나, 미들웨어 레이어를 거쳐 핸들러나 비즈니스 로직으로 전달되는 요청별 GUID를 생성하고자 하는 경우 발생할 수 있다.</p> <p>이러한 경우에 대응하여, <code>context</code> 패키지의 다양한 팩토리 함수 중 값을 저장할 수 있는 컨텍스트를 만드는 함수인 <code>context.WithValue()</code>가 존재한다.
이 함수는 세 개의 파라미터를 받는다. 파라미터는 각각 parent 컨텍스트와, 값을 저장할 key와, 값이다.
그리고 parent 컨텍스트를 감싸며 key-value 페어가 존재하는 child 파라미터를 리턴한다.
이 때 각각 key와 value의 타입은 <code>interface&#123;&#125;</code>로 선언되어 있다.</p> <br/> <p>만약 어떤 컨텍스트나 해당 컨텍스트의 아무 parent 컨텍스트 중 값이 저장되어 있는지 확인하려면, <code>context.Context</code>의 <code>Value()</code> 메소드를 사용하면 된다.
이 메소드는 Key를 파라미터로 받아서 해당 Key에 대한 값을 리턴한다.
만약 해당 Key에 대한 값이 존재하지 않는다면 <code>nil</code>이 리턴된다.
타입이 <code>interface&#123;&#125;</code>이기 때문에, 반환받은 값이 유효한지 확인하려면 comma ok idiom을 사용해주어야 한다.</p> <blockquote><p>기본적으로 context chain에 저장된 값을 검색하려면 <em>linear search</em>를 사용할 수밖에 없다.
만약 컨텍스트에 저장된 값이 많지 않다면 성능상의 문제가 크진 않겠지만, 컨텍스트에 값을 너무 많이 저장하게 되면 성능이 뚝뚝 떨어질 것이다.
요청을 처리하기 위해 context chain에 값을 필요 이상으로 많이 집어넣어야 한다면, 리팩토링을 한번 해보는 게 좋을 것이다.</p></blockquote> <br/> <p>컨텍스트에 저장할 key든 value든 타입이 <code>interface&#123;&#125;</code>이기 때문에 아무거나 다 넣을 수 있긴 하지만, <code>map</code>처럼 key나 value의 타입을 고정해주는 것이 좋다(특히 key는 더더욱! 비교가 가능한 타입으로 고정해주는 것이 좋다).</p> <p>아래 예제에서는 <code>int</code>를 기반으로, export되지 않은 타입을 생성해주었다.</p> <pre class="language-go"><!></pre> <p>만약 key의 타입으로 string이나 다른 public type을 사용한다면, 다른 패키지에서 동일한 키값이 생성될 수 있고, 이는 충돌로 이어질 수 있다.
그렇게 되면 컨텍스트에서 다른 패키지에서 쓴 데이터를 읽는 등, 요상한 문제가 발생하여 디버그가 힘들어지기도 한다.</p> <br/> <p>이렇게 export되지 않는 key의 타입을 선언해주고 나면, export되지 않는 상수를 하나 선언해준다.</p> <pre class="language-go"><!></pre> <p>위 상수와 타입 모두 export되지 않았기 때문에, 패키지 바깥에서 이 데이터를 확인할 방법이 없다.
따라서 컨텍스트에 겹치는 key에 덮어쓰거나 접근할 방법이 없을 것이며, 충돌도 발생하지 않을 것이다.
또한 만약 컨텍스트에 여러 개의 값을 저장해야 한다면 <code>iota</code>를 사용할 수도 있을 것이다.
기본적으로 상수의 값이 의미있는 것이 아니라 단지 구분을 하기 위해서만 사용되기 때문에, <code>iota</code>를 사용하기 매우 적절한 경우라고 할 수 있다.</p> <br/> <p>다음으로는 컨텍스트에 값을 집어넣거나, 값을 읽어오는 API를 작성해줄 것이다.
패키지 외부의 코드에서 컨텍스트의 값을 읽고 써야 할 필요가 있는 경우, 이 함수들을 export한다.
값을 저장하는 컨텍스트를 생성하는 함수의 이름은 <code>ContextWith</code>로 시작해야 한다.
또한 컨텍스트로부터 값을 읽어와서 리턴하는 함수의 이름은 <code>FromContext</code>로 끝나야 한다.</p> <p>컨텍스트에 값을 읽고 쓰는 함수들의 예제를 살펴보도록 하자.</p> <pre class="language-go"><!></pre> <br/> <p>사용자 정보를 관리하는 코드를 작성했으니, 사용하는 방법을 알아볼 것이다.
먼저, 아래 예제는 쿠키에서 유저 ID를 추출하는 미들웨어이다.</p> <pre class="language-go"><!></pre> <p>해당 미들웨어 내에서는 <code>user</code>의 쿠키 값을 추출한 후, <code>Context()</code> 메소드를 사용해 요청의 컨텍스트를 얻는다.
...
마지막으로 <code>http.Handler</code>의 다음 함수를 호출해준다.</p> <br/> <pre class="language-go"><!></pre> <p>위 예제의 핸들러에서는 <code>Context()</code> 메소드로 요청에 대한 컨텍스트를 획득하여,
해당 컨텍스트로부터 <code>UserFromContext()</code> 함수를 통해 유저 정보를 추출한다. 그리고 비즈니스 로직을 호출한다.</p> <p>파라미터를 통해 값을 넘기기보단, 이런 방식을 통해 컨텍스트에 값을 저장하는게 더 나을 만한 상황도 존재한다. 앞서 말했던 GUID를 추적하는 경우이다.
이 정보는 비즈니스 로직보단 어플리케이션 관리를 위해 사용된다.
이러한 방식을 사용하면 GUID에 대해 알 필요가 없는 비즈니스 로직이 이 정보를 볼 수 없게 되며, 로그를 작성하거나 다른 서버에 연결할 때 사용할 수 있다.</p> <br/> <p>다음은 컨텍스트가 서비스에서 서비스로 전달될 때, GUID를 포함하여 이 값을 추적하고 로그를 기록하는 예제이다.</p> <pre class="language-go"><!></pre> <p><code>Middleware()</code> 함수는 요청으로부터 GUID를 추출하거나 새로은 GUID를 만들고, 컨텍스트에 이 값을 저장하여 새로운 컨텍스트를 생성한다.
그리고 다음의 Middleware chain을 호출한다.</p> <p><code>Logger</code> 구조체에는 컨텍스트와 문자열을 받아서 제네릭하게 로그를 남기는 메소드가 있다.
만약 컨텍스트에 GUID가 있다면, 로그 메시지의 앞에 추가적으로 출력한다. <code>Request</code> 함수는 다른 서비스를 호출하기 위해 사용되며, 컨텍스트에 GUID가 있다면 Request header에 GUID가 추가된 <code>*http.Request</code> 인스턴스를 리턴한다.</p> <br/> <p>Interface 포스트에서 보았던 것처럼, 인터페이스를 통해 Dependency Injection을 하는 패턴을 사용해볼 것이다.
이 방법을 통해 비즈니스 로직으로부터 불필요한 정보를 분리할 수 있다.</p> <p>먼저 <code>Logger</code> 인터페이스를 정의하고, request decorator를 나타내는 함수 타입과, 이에 관련된 비즈니스 로직 구조체를 만들어준다.</p> <pre class="language-go"><!></pre> <p>이제 비즈니스 로직을 구현해준다.</p> <pre class="language-go"><!></pre> <p>GUID가 <code>Logger</code>와 request decorator에게 전달되는 과정에서 비즈니스 로직은 이를 인식하지 못하며,
프로그램 로직에 필요한 데이터와 프로그램 관리에 필요한 데이터를 분리할 수 있다.
코드에서 연관성을 확인할 수 있는 유일한 부분은 바로 <code>main()</code> 함수이다.</p> <pre class="language-go"><!></pre> <br/> <p>전체 코드는 <a href="https://github.com/jhseoeo/Learning-golang/tree/master/12-context/values" rel="nofollow">여기</a>에서 확인할 수 있다.</p> <br/><br/> <h2 id="references"><a aria-hidden="true" tabindex="-1" href="#references"><span class="icon icon-link"></span></a>References</h2> <hr/> <center><p>[</p> <!> ](https://learning.oreilly.com/library/view/learning-go/9781492077206/) <br/> [Jon Bodner, 『Learning Go』, O'Reilly Media, Inc.](https://learning.oreilly.com/library/view/learning-go/9781492077206/)</center> <br/><br/>`,1);function Rn(T){var _=cn(),p=n(an(_),28),G=s(p);t(G,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">logic</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> info <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">string</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(p);var o=n(p,10),U=s(o);t(U,()=>`<code class="language-go">ctx <span class="token operator">:=</span> context<span class="token punctuation">.</span><span class="token function">Background</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
result<span class="token punctuation">,</span> err <span class="token operator">:=</span> <span class="token function">logic</span><span class="token punctuation">(</span>ctx<span class="token punctuation">,</span> <span class="token string">"a string"</span><span class="token punctuation">)</span></code>`),a(o);var e=n(o,18),H=s(e);t(H,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">Middleware</span><span class="token punctuation">(</span>handler http<span class="token punctuation">.</span>Handler<span class="token punctuation">)</span> http<span class="token punctuation">.</span>Handler <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(e);var c=n(e,8),I=s(c);t(I,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">handler</span><span class="token punctuation">(</span>w http<span class="token punctuation">.</span>ResponseWriter<span class="token punctuation">,</span> r <span class="token operator">*</span>http<span class="token punctuation">.</span>Request<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	ctx <span class="token operator">:=</span> r<span class="token punctuation">.</span><span class="token function">Context</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
...
<span class="token punctuation">&#125;</span></code>`),a(u);var l=n(u,23),F=s(l);t(F,()=>`<code class="language-go"><span class="token keyword">package</span> main
...
<span class="token keyword">func</span> <span class="token function">fastServer</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">*</span>httptest<span class="token punctuation">.</span>Server <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
	<span class="token keyword">return</span> s
<span class="token punctuation">&#125;</span></code>`),a(l);var k=n(l,10),B=s(k);t(B,()=>`<code class="language-go"><span class="token keyword">package</span> main
...
<span class="token punctuation">&#125;</span></code>`),a(k);var i=n(k,14),E=s(i);t(E,()=>`<code class="language-go"><span class="token keyword">package</span> main
...
<span class="token punctuation">&#125;</span>
</code>`),a(i);var r=n(i,8),M=s(r);t(M,()=>'<code class="language-bash">$ go build ./<span class="token punctuation">..</span>.</code>'),a(r);var d=n(r,4),N=s(d);t(N,()=>`<code class="language-bash">$ ./context-cancel <span class="token boolean">false</span>
fast result: ok
slow result: Slow response
<span class="token keyword">done</span> with both</code>`),a(d);var g=n(d,6),A=s(g);t(A,()=>`<code class="language-bash">$ ./context-cancel <span class="token boolean">true</span>
fast result: error
...
<span class="token keyword">done</span> with both</code>`),a(g);var f=n(g,4),D=n(s(f),4),O=s(D);t(O,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">slowServer_rewrite</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">*</span>httptest<span class="token punctuation">.</span>Server <span class="token punctuation">&#123;</span>
	s <span class="token operator">:=</span> httptest<span class="token punctuation">.</span><span class="token function">NewServer</span><span class="token punctuation">(</span>http<span class="token punctuation">.</span><span class="token function">HandlerFunc</span><span class="token punctuation">(</span><span class="token keyword">func</span><span class="token punctuation">(</span>w http<span class="token punctuation">.</span>ResponseWriter<span class="token punctuation">,</span> r <span class="token operator">*</span>http<span class="token punctuation">.</span>Request<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
		ctx <span class="token operator">:=</span> r<span class="token punctuation">.</span><span class="token function">Context</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
	...
	<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
	<span class="token keyword">return</span> s
<span class="token punctuation">&#125;</span></code>`),a(D),S(2),a(f);var x=n(f,64),V=s(x);t(V,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">longRunningThing</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> data <span class="token builtin">int</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">int</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token keyword">func</span> <span class="token function">longRunningThingManager</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> data <span class="token builtin">int</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">int</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(x);var h=n(x,12),K=s(h);t(K,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	ctx <span class="token operator">:=</span> context<span class="token punctuation">.</span><span class="token function">Background</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
	ctx<span class="token punctuation">,</span> cancel <span class="token operator">:=</span> context<span class="token punctuation">.</span><span class="token function">WithCancel</span><span class="token punctuation">(</span>ctx<span class="token punctuation">)</span>
...
<span class="token punctuation">&#125;</span></code>`),a(h);var w=n(h,37),j=s(w);t(j,()=>'<code class="language-go"><span class="token keyword">type</span> userKey <span class="token builtin">int</span></code>'),a(w);var y=n(w,8),J=s(y);t(J,()=>'<code class="language-go"><span class="token keyword">const</span> key userKey <span class="token operator">=</span> <span class="token number">1</span></code>'),a(y);var b=n(y,10),$=s(b);t($,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">ContextWithUser</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> user <span class="token builtin">string</span><span class="token punctuation">)</span> context<span class="token punctuation">.</span>Context <span class="token punctuation">&#123;</span>
...
<span class="token keyword">func</span> <span class="token function">Middleware</span><span class="token punctuation">(</span>h http<span class="token punctuation">.</span>Handler<span class="token punctuation">)</span> http<span class="token punctuation">.</span>Handler <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),a(m);var v=n(m,6),Q=s(v);t(Q,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token punctuation">(</span>c Controller<span class="token punctuation">)</span> <span class="token function">handleRequest</span><span class="token punctuation">(</span>w http<span class="token punctuation">.</span>ResponseWriter<span class="token punctuation">,</span> r <span class="token operator">*</span>http<span class="token punctuation">.</span>Request<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	ctx <span class="token operator">:=</span> r<span class="token punctuation">.</span><span class="token function">Context</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
	...
	w<span class="token punctuation">.</span><span class="token function">Write</span><span class="token punctuation">(</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token function">byte</span><span class="token punctuation">(</span>result<span class="token punctuation">)</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),a(v);var C=n(v,10),X=s(C);t(X,()=>`<code class="language-go"><span class="token keyword">package</span> tracker
...
<span class="token punctuation">&#125;</span></code>`),a(C);var R=n(C,12),Y=s(R);t(Y,()=>`<code class="language-go"><span class="token keyword">type</span> RequestDecorator <span class="token keyword">func</span><span class="token punctuation">(</span><span class="token operator">*</span>http<span class="token punctuation">.</span>Request<span class="token punctuation">)</span> <span class="token operator">*</span>http<span class="token punctuation">.</span>Request
...
<span class="token punctuation">&#125;</span></code>`),a(R);var q=n(R,4),Z=s(q);t(Z,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token punctuation">(</span>bl BusinessLogic<span class="token punctuation">)</span> <span class="token function">businessLogic</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> user <span class="token builtin">string</span><span class="token punctuation">,</span> data <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token builtin">string</span><span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(q);var W=n(q,4),nn=s(W);t(nn,()=>`<code class="language-go">bl <span class="token operator">:=</span> BusinessLogic<span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(W);var L=n(W,13),sn=n(s(L),2);on(sn,{alt:"Learning Go Book Cover",src:"https://learning.oreilly.com/covers/urn:orm:book:9781492077206/400w/"}),S(3),a(L),S(3),pn(T,_)}export{Rn as default,en as metadata};
