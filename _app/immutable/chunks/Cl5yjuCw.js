import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{s as n,f as v,c as s,r as a,n as D}from"./CVx5jffJ.js";import{f as x,a as M}from"./2cXyNWGb.js";import{h as t}from"./D6IIVJDJ.js";const I={title:"Golang으로 WebRTC Mesh Server 구현하기",date:"2023-02-18T00:00:00.000Z",excerpt:"Golang을 통해 M:N WebRTC 시그널링 서버를 만들어보았습니다",categories:["WebRTC","Golang","Signaling Server"],coverImage:"/post_img/Networking/WebRTC/cover.png",coverWidth:16,coverHeight:9,indexed:!0,exposed:!0},{title:G,date:N,excerpt:j,categories:L,coverImage:E,coverWidth:B,coverHeight:A,indexed:F,exposed:O}=I;var R=x(`<p>Golang으로 하는 첫 프로젝트로, WebRTC many-to-many 시그널링 서버를 만들게 되었다.</p> <br/><br/> <h2 id="basic-11-signaling"><a aria-hidden="true" tabindex="-1" href="#basic-11-signaling"><span class="icon icon-link"></span></a>Basic 1:1 Signaling</h2> <p>우선 가장 기본적인 예제인, 1:1 연결 시그널링 서버의 예제부터 구현하였다. 1:1 시그널링 서버는 언어를 막론하고 예제가 많이 존재한다.
본인 또한 참고한 예제가 있다. <a href="https://github.com/shanet/WebRTC-Example" rel="nofollow">https://github.com/shanet/WebRTC-Example</a>의 예제를 확인하면 간단한 웹 클라이언트와, Node.js 및 WebSocket 기반의 간단한 시그널링 서버를 찾아볼 수 있다.</p> <br/> <p>서버쪽 소스코드는 너무나도 간단하다. ICE나 SDP 등 WebRTC 연결 정보를 전달해주는 역할을 하는 코드는 이 정도밖에 안된다.</p> <pre class="language-javascript"><!></pre> <p>한쪽 피어가 보낸 메시지를 다른 피어한테 웹소켓을 통해 전송해주는 로직으로, 사실상 채팅 서버와 거의 동일하다고 볼 수 있다.
그래서 Go로 작성된 웹소켓 기반 채팅서버 예제를 참조하여 금방 시그널링 서버를 구현할 수 있었다. <a href="https://github.com/jos-/gofiber-websocket-chat-example" rel="nofollow">https://github.com/jos-/gofiber-websocket-chat-example</a>의 예제를 참조하여 동일 클라이언트 코드를 사용하는 Go 예제로 옮겨보았다.</p> <p>프로젝트는 <a href="https://github.com/gofiber/fiber" rel="nofollow">GoFiber</a>의 <a href="https://github.com/gofiber/websocket" rel="nofollow">웹소켓 extension</a>을 사용하여 진행하였다.
...
괜찮은 대체제를 찾는 겸, Fiber 연습하는 겸 GoFiber WebSocket extension를 사용하게 되었다.</p> <br/> <p>먼저, 다음과 같이 유저가 접속하거나 나갈 때 데이터를 전송할 채널과, 채널에 들어온 데이터를 받아서 처리할 고루틴을 작성해주었다.</p> <pre class="language-go"><!></pre> <p>각각 <code>register</code> 채널에 유저 정보가 입력되면 유저를 전역변수 <code>clients</code>에 추가하고, <code>unregister</code> 채널에 입력되면 유저를 제거한다. <code>broadcast</code> 채널에 메시지가 입력되면 메시지를 모든 유저에게 전달한다.</p> <p><code>*ws.Conn</code> 타입의 인스턴스는 각 웹소켓 클라이언트에 해당하는 것으로, <code>*ws.Conn</code>의 <code>ReadMessage()</code> 등을 호출하여 데이터를 읽거나 쓸 수 있다.</p> <br/> <p>유저의 웹소켓 세션에 관련된 부분은 다음과 같다.</p> <pre class="language-go"><!></pre> <p>먼저 웹소켓을 사용할 수 있는지 검사한 후, 사용 가능하다면 웹소켓 연결을 생성한다.
웹소켓 연결이 생성되면 <code>register</code> 채널에 클라이언트 정보를 보내고, defer문을 통해 클라이언트가 웹소켓 연결을 끊으면 <code>unregister</code> 채널에 클라이언트 정보를 보낼 수 있게끔 하였다.</p> <p>1:1 연결에서는 이러한 100줄 정도의 코드만으로도 시그널링 서버를 구현할 수 있다.</p> <br/> <p>전체 코드는 <a href="https://github.com/jhseoeo/webrtc-mesh-server/tree/5d435d745a48ba3be0c1299163029364ec31a3bf" rel="nofollow">https://github.com/jhseoeo/webrtc-mesh-server/tree/5d435d745a48ba3be0c1299163029364ec31a3bf</a>에서 찾아볼 수 있다.</p> <br/><br/> <h2 id="개선할-점"><a aria-hidden="true" tabindex="-1" href="#개선할-점"><span class="icon icon-link"></span></a>개선할 점</h2> <p>일단 M:M으로 나아가기에 앞서, 위 예제에서 몇 가지 문제점을 찾아볼 수 있다.</p> <ul><li>서버가 1:1 연결 하나만 처리할 수 있음… 세션 정도는 만들어줄 필요가 있음.</li> <li>클라이언트 코드도 마찬가지로 상대방이 한 명이라고 가정하여 작성되어 있음. 여러 명의 유저와 통신할 수 있게끔 수정해야 함.</li> <li>유저가 나갈 때에 대한 처리가 부족함. 한 유저가 나가면 <code>RTCPeerConnection</code>을 통해 유저가 나갔다는 사실을 전달받을 수 있겠지만, 그걸 통해 <code>&lt;video&gt;</code> element를 지운다던지 하는 처리가 전혀 없음.</li> <li>고루틴을 여러 개 띄워서 전역 변수인 <code>client</code>에 읽기/쓰기를 시도하는 반면, <code>client</code>는 뮤텍스 등을 통해 보호되지 않음. 이 때문에 새로고침을 막 눌러서 연결을 빡세게 시도하면 data race로 인한 panic이 일어나서 서버가 곧잘 죽는 것을 확인할 수 있음</li> <li>모든 컴포넌트가 <code>main.go</code> 한 파일에 몰려있음. 리팩토링을 통해 디커플링하는 겸 변수 이름도 Convention에 맞게 다시 지어주면 좋을 듯 함.</li></ul> <p>또한 Mesh 형태의 many-to-many 연결을 구현하기 위해서는 시그널링 서버에서 특정 유저한테만 메시지를 보낼 수 있는 방법이 추가되어야 한다. WebRTC 연결을 생성하려는 대상이 아닌 다른 대상에게까지 메시지가 전달되면 오류가 발생하여 연결 설정이 제대로 안될 수 있기 때문이다.</p> <p>다음으로 다룰 예제에서 위 사항을 충족시킨 코드를 확인할 수 있을 것이다.</p> <br/><br/> <h2 id="many-to-many-mesh-server"><a aria-hidden="true" tabindex="-1" href="#many-to-many-mesh-server"><span class="icon icon-link"></span></a>Many-to-Many Mesh Server</h2> <p><code>main.go</code>의 변경사항이다.</p> <pre class="language-go"><!></pre> <p>우선, 이전에 전역변수로 선언되던 <code>clients</code>는 각 세션별로 존재하며, 각 세션은 <code>Hub</code>에서 만들어진다.<br/> 또한 클라이언트별 웹소켓 루프를 별도의 함수로 지정하였다.</p> <br/> <p>웹소켓 루프 함수를 살펴보자.</p> <pre class="language-go"><!></pre> <p>클라이언트가 접속하면 자신의 UUID를 보낸다. 클라이언트가 접속한 세션 정보와 이 UUID 정보를 바탕으로 <code>RegisterUser()</code> 메소드를 호출하면 클라이언트가 세션에 등록된다. 클라이언트가 접속을 종료할 때는 <code>UnregisterUser()</code>를 호출하여 유저의 퇴장을 처리한다.</p> <p>이후, ICE나 SDP 등 시그널링 메시지를 처리하는 단계로 진입한다. <code>SendSignallingMessage()</code> 메소드를 통해 UUID로 구분하여 특정 대상에게 시그널링 메시지를 전달할 수 있다.</p> <br/> <p>다음으로, <code>SessionDataStore</code>에 대해 살펴보자.</p> <pre class="language-go"><!></pre> <p><code>SessionDataStore</code>는 세션별로 유저들의 정보가 저장되는 자료구조로, <code>GetSessionData()</code>, <code>SetUserData()</code>, <code>DeleteUserData()</code> 메소드를 통해 접근할 수 있다. <code>RWMutex</code>를 설정하여 여러 고루틴에서 동시에 접근하여 생길 수 있는 Data race 문제를 해결하였다.</p> <br/> <p>채널을 관리하는 <code>Hub</code>쪽을 살펴보자.</p> <pre class="language-go"><!></pre> <p><code>Hub</code>는 각 세션 별로 채널을 만들고 관리하는 역할이다. <code>Hub</code>의 메소드는 기본적으로 호출이 되면 유저의 세션에 따라 대응되는 채널에 정보를 집어넣는다. 이 채널은 각 세션별 고루틴으로 연결되어, 해당 고루틴에서 처리된다.</p> <pre class="language-go"><!></pre> <p><code>RegisterUser()</code>에서는 채널에 등록할 유저 정보를을 넣기에 앞서 채널이 있는지 검사하여, 없다면 각 세션의 고루틴 및 채널을 생성한다. <code>UnregisterUser()</code>에서는 세션에 더이상 유저가 없는 경우 채널을 삭제하는 로직까지 추가된다.</p> <br/> <p>마지막으로 각 세션의 고루틴을 실행하는 <code>RunSessionLoop()</code> 함수를 살펴보자.</p> <pre class="language-go"><!></pre> <p>채널 목록 및 <code>MakeSessionDataStore()</code>으로 세션별 데이터 저장소를 생성하고 for~switch loop으로 채널에서 데이터를 읽어와 처리한다.</p> <br/> <p>전체 코드는 <a href="https://github.com/jhseoeo/webrtc-mesh-server" rel="nofollow">https://github.com/jhseoeo/webrtc-mesh-server</a>에서 찾아볼 수 있다.</p> <br/><br/> <h2 id="후기"><a aria-hidden="true" tabindex="-1" href="#후기"><span class="icon icon-link"></span></a>후기</h2> <p>many-to-many 만들 때 시그널링 메시지를 브로드캐스팅으로 보내버려서 피어 수가 3명이 넘어가면 연결이 제대로 안됐었는데, 뭐가 문제인지 찾는 데 한참 걸렸다.</p> <p>개인적으로 WebRTC 좀 친다고 생각했었는데 아직 갈 길이 먼 것 같다..</p>`,1);function J(g){var r=R(),p=n(v(r),13),f=s(p);t(f,()=>`<code class="language-javascript"><span class="token keyword">const</span> wss <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">WebSocketServer</span><span class="token punctuation">(</span><span class="token punctuation">&#123;</span> server<span class="token operator">:</span> httpsServer <span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
wss<span class="token punctuation">.</span><span class="token function-variable function">broadcast</span> <span class="token operator">=</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token parameter">data</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span><span class="token punctuation">;</span></code>`),a(p);var o=n(p,10),w=s(o);t(w,()=>`<code class="language-go"><span class="token keyword">type</span> channelSet <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
...
<span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token operator">...</span>
<span class="token punctuation">&#125;</span></code>`),a(o);var e=n(o,10),m=s(e);t(m,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	<span class="token operator">...</span>
	...
	app<span class="token punctuation">.</span><span class="token function">Get</span><span class="token punctuation">(</span><span class="token string">"/ws/"</span><span class="token punctuation">,</span>
			...
			ws<span class="token punctuation">.</span><span class="token function">New</span><span class="token punctuation">(</span><span class="token keyword">func</span><span class="token punctuation">(</span>c <span class="token operator">*</span>ws<span class="token punctuation">.</span>Conn<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
				...
				<span class="token punctuation">&#125;</span>
			<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),a(e);var c=n(e,30),h=s(c);t(h,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	app<span class="token punctuation">.</span><span class="token function">Get</span><span class="token punctuation">(</span><span class="token string">"/ws/:session"</span><span class="token punctuation">,</span>
		...
		<span class="token punctuation">&#125;</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
<span class="token punctuation">&#125;</span></code>`),a(c);var u=n(c,8),y=s(u);t(y,()=>`<code class="language-go"><span class="token comment">// Websocket Session Loop for each client</span>
<span class="token keyword">func</span> <span class="token function">WebsocketConnectionLoop</span><span class="token punctuation">(</span>hub <span class="token operator">*</span>MessageHub<span class="token punctuation">,</span> conn <span class="token operator">*</span>ws<span class="token punctuation">.</span>Conn<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(u);var l=n(u,10),b=s(l);t(b,()=>`<code class="language-go"><span class="token comment">// Client struct type. you can add any data here</span>
...
<span class="token keyword">func</span> <span class="token function">MakeSessionDataStore</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">*</span>SessionDataStore <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(l);var k=n(l,8),S=s(k);t(S,()=>`<code class="language-go"><span class="token operator">...</span>
...
<span class="token punctuation">&#125;</span></code>`),a(k);var i=n(k,4),U=s(i);t(U,()=>`<code class="language-go">
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>h <span class="token operator">*</span>Hub<span class="token punctuation">)</span> <span class="token function">UnregisterUser</span><span class="token punctuation">(</span>session SessionName<span class="token punctuation">,</span> uuid UUIDType<span class="token punctuation">,</span> client Client<span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">if</span> toDetete <span class="token punctuation">&#123;</span>
		...
		h<span class="token punctuation">.</span>mutex<span class="token punctuation">.</span><span class="token function">Unlock</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
	<span class="token punctuation">&#125;</span>
<span class="token punctuation">&#125;</span></code>`),a(i);var d=n(i,8),C=s(d);t(C,()=>`<code class="language-go"><span class="token keyword">func</span> <span class="token function">RunSessionLoop</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">(</span><span class="token operator">*</span>ChannelSet<span class="token punctuation">,</span> <span class="token builtin">error</span><span class="token punctuation">)</span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(d),D(15),M(g,r)}export{J as default,I as metadata};
