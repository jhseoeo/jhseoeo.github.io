import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{n as l}from"./CVx5jffJ.js";import{f as a,a as t}from"./2cXyNWGb.js";const u={title:"Timeseries data abstraction",date:"2025-11-08",categories:["Backend"],coverImage:"/images/default-cover.jpg",coverWidth:16,coverHeight:9,excerpt:"",indexed:!1,exposed:!0};var s=a(`<p><a href="https://netflixtechblog.com/introducing-netflix-timeseries-data-abstraction-layer-31552f6326f8">Introducing Netflix’s TimeSeries Data Abstraction Layer</a></p> <p>양이 왜이렇게 많아</p> <h2>Introduction</h2> <p>넷플릭스 형님들께서 low-latency로 대량의 이벤트 데이터를 처리할 수 있게끔 만든 추상화 계층</p> <p>메트릭, 히스토그램 등 일반적인 준실시간 유즈케이스로 사용하는 게 아니라, 임시적인 이벤트 데이터를 높은 throughput과 낮은 latency로 처리하기 위해 사용</p> <h2>어따씀</h2> <ul><li>마이크로서비스간 통신 로깅 및 인사이트</li> <li>사용자가 페이지에서 하는 검색, 플래이백 등 모든 User Interaction을 기록하고 추천 알고리즘에 전달</li> <li>외에도 몇 개 있으나… 조금이라도 실시간성이 중요한 기능에는 사용하지 않는 듯</li></ul> <h2>Challenges</h2> <ul><li>여기서 처리하고자 하는 이벤트의 예시는 비디오 시청 이벤트, 인게임 이벤트(이녀석들 게임도 하나보네요), 넓게는 마이크로서비스 네트워크 이벤트등도 포함될 수 있음</li> <li>근데 이런거 처리하려면 다음과 같은 요구사항이 필요함 <ul><li>가용성 유지하면서 초당 1000만 건의 write를 처리</li> <li>페타바이트 단위 데이터를 저장하고, 2자릿수 밀리초 내에 결과를 반환할 수 있어야 함</li> <li>세계 어디서든 읽고쓰기 가능</li> <li>유동적인 파티셔닝 전략</li> <li>트래픽 급증 대응</li> <li>근데 이제 최대한 저렴하게</li></ul></li> <li>넷플릭스 정도 되는 서비스는 역시 단위부터가 다르다… 짐바브웨 화폐를 보는 듯한</li></ul> <p></p> <h2>Data Model</h2> <img src="/images/Timeseries_data_abstraction/9c7f07f1f8dfcb9b.png" alt="" class="responsive-image"/> <p>적당한 캡슐화와 효율적인 쿼리가 가능한 모델</p> <ul><li>Event Item: 특정 이벤트에 대한 데이터를 저장하기 위한 key-value 페어. e.g. &#123;“device_type”: “ios”&#125;</li> <li>Event: 여러 Event Item으로 이루어짐. 타임스탬프(event_time)와 식별자(event_id)의를 조합하여 고유한 키 생성</li> <li>Time Series ID: 데이터 보존 기간 동안 저장되어 있는 한 개 이상의 Event 모음</li> <li>Namespace: 대충 예상 가는 그것. Time Series ID로 이루어진 전체 데이터세트의 모음이고, 네임스페이스별로 설정 튜닝 가능</li></ul> <p></p> <h2>API</h2> <ul><li>Write <ul><li>WriteEventRecords: fire-and-forgot 및 약간의 손실을 허용하는 endpoint</li> <li>WriteEventRecordsSync: 위 요청의 sync 버전. 정확히 write를 해야 할 때 사용할 듯</li></ul> <details><summary>요청 JSON</summary> <pre class="language-json"><code class="language-json">&#123;
