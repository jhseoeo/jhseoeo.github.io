import"./Bzak7iHL.js";import"./69_IOA4Y.js";import{s as t,f as B,c as p,r as o,n as D}from"./CVx5jffJ.js";import{f as u,a as e}from"./2cXyNWGb.js";import{h as c}from"./D6IIVJDJ.js";import{C as l}from"./BQl3WXse.js";import{I as _}from"./BVXqo7x9.js";const I={title:"팩토리, 레포지토리, 서비스",date:"2023-07-16T00:00:00.000Z",excerpt:"Factories, Repositories, and Services",categories:["Golang","Backend","Architecture","Domain Driven Design"],coverImage:"/post_img/Backend/Architecture/DDD/cover.png",coverWidth:16,coverHeight:9,indexed:!1,exposed:!0},{title:L,date:Q,excerpt:z,categories:H,coverImage:V,coverWidth:Z,coverHeight:J,indexed:X,exposed:Y}=I;var $=u('<pre class="language-go"><!></pre>'),E=u('<pre class="language-go"><!></pre>'),P=u('<pre class="language-go"><!></pre>'),T=u('<pre class="language-go"><!></pre>'),R=u('<pre class="language-go"><!></pre>'),j=u('<pre class="language-go"><!></pre>'),M=u('<pre class="language-go"><!></pre>'),U=u('<pre class="language-go"><!></pre>'),N=u(`<h2 id="팩토리-패턴"><a aria-hidden="true" tabindex="-1" href="#팩토리-패턴"><span class="icon icon-link"></span></a>팩토리 패턴</h2> <hr/> <p>팩토리 패턴은 주로 객체지향 프로그래밍에서 찾아볼 수 있으며, 다른 오브젝트를 생성하는 주된 책임을 가지고 있는 개체로 정의된다.
객체의 기본 속성을 설정하는 데 유용하며, 일반적으로 객체의 생성 외에 다른 목적을 가지면 안된다.</p> <p>Go는 객체지향 언어가 아님에도 팩토리 패턴은 꽤 유용하다.
아래 예제는 팩토리 패턴의 간단한 예제이다.</p> <!> <p>위 예제에서는 <code>Car</code> 인터페이스를 충족하는 <code>BMW</code>와 <code>Tesla</code> 구조체를 정의하였고, <code>Car</code> 오브젝트의 필드값을 초기화하여 반환하는 팩토리 함수 <code>BuildCar</code>를 정의하였다.
...
누군가 업무 시간이 지나고 예약하려 하는 경우, 다음과 같이 팩토리 함수를 통해 예외를 발생시킬 수 있다.</p> <!> <p>위 예제는 엔티티를 생성하는 팩토리 함수를 보여주며, 엔티티의 식별자는 팩토리에서 생성된다.</p> <p>엔티티 팩토리에 대헤 조금 더 살펴보자.</p> <br/> <h3 id="엔티티-팩토리"><a aria-hidden="true" tabindex="-1" href="#엔티티-팩토리"><span class="icon icon-link"></span></a>엔티티 팩토리</h3> <p>앞선 포스트에서 설명하였듯 엔티티에는 식별자가 있고, 인스턴스화하기 위한 최소한의 요구사항이 존재한다.
...
DDD를 사용한다면 위와 같은 시스템을 구축하는 것이 좋다.</p> <br/> <p>이전 장의 예약 시스템 예제에서 계속하여, 미용실 예약 정보를 데이터베이스에 저장하려 한다. 먼저 레포지토리 계층에 해당하는 인터페이스를 정의한다.</p> <pre class="language-go"><!></pre> <p>이 인터페이스는 <code>Booking</code> 팩토리 및 서비스 계층과 같은 계층에 정의되었는데, 그 이유는 다음 장에서 설명한다.</p> <p>PostgreSQL 데이터베이스를 위한 간단한 레포지토리 레이어를 구현해보자.</p> <!> <p>코드에서 볼 수 있듯, 데이터베이스와 상호작용하는 것은 꽤 간단하며, 도메인 로직이 들어가지 않는다.
...
엔티티 혹은 밸류 오브젝트로 모델링하는 좋은 방법을 찾을 수 없는 프로세스가 있을 때가 있는데, 이 경우 도메인 서비스를 사용하면 좋다</p> <p>도메인 서비스의 규칙을 정의하기는 좀 애매한 감이 있지만, 주의해야 할 점이 있다.</p> <ul><li>작성되는 코드는 한 도메인 내에서 중요한 비즈니스 로직을 수행해야 한다.</li> <li>한 도메인 객체를 다른 도메인 객체로 변환한다.</li> <li>값을 계산하기 위해 두 개 이상의 도메인 객체의 속성을 사용한다.</li></ul> <p>서비스는 DDD의 다른 모든 요소들과 마찬가지로 제한된 컨텍스트 안에서 유비쿼터스 언어를 통해 표현되어야 한다.</p> <p>서비스가 유용하게 쓰일 수 있는 예제를 살펴보자. 엔티티 내에 이런 코드가 있다고 가정해보자.</p> <!> <p>괜찮아 보이는 코드지만 문제가 있다. <code>ShoppingCart</code>의 구현에서 다른 엔티티를 참조하고 있고, 실제로 <code>ShoppingCart</code>에 속하지 않는 비즈니스 로직이 포함되어 있다.
이런 문제를 해결하기 위해 도메인 서비스를 사용할 수 있다.</p> <!> <p>두 엔티티에 모두 접근할 수 있는 도메인 로직이 도메인 서비스에 작성되었다.
...
다만 도메인 로직은 애플리케이션 서비스가 아니라 도메인 서비스에 작성되어야 한다.</p> <p>어플리케이션 서비스는 보통 그렇게 길지 않다. 일반적으로 트랜잭션 등 조정을 위해 사용되며, 다른 로직은 어플리케이션 레이어 밑으로 내려가야 한다. 또한 보한 문제를 해결하기도 한다.</p> <p>예약 예제를 통해 애플리케이션 서비스를 살펴보자.</p> <!> <p>위 코드에서는 기본적인 인증을 수행하고, 도메인 레이어와 레포지토리 레이어로 애플리케이션 서비스를 구성한다.
...
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var g=t(d,8);l(g,{children:(s,i)=>{var n=E(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token punctuation">&#125;</span>
</code>`),o(n),e(s,n)},$$slots:{default:!0}});var k=t(g,37),x=p(k);c(x,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token punctuation">&#125;</span></code>`),o(k);var y=t(k,6);l(y,{children:(s,i)=>{var n=P(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>p PostgresRepository<span class="token punctuation">)</span> <span class="token function">DeleteBooking</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> booking Booking<span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var f=t(y,27);l(f,{children:(s,i)=>{var n=T(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>s <span class="token operator">*</span>ShoppingCart<span class="token punctuation">)</span> <span class="token function">AddToCard</span><span class="token punctuation">(</span>p Product<span class="token punctuation">)</span> <span class="token builtin">bool</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">return</span> <span class="token boolean">true</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var b=t(f,4);l(b,{children:(s,i)=>{var n=R(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>c <span class="token operator">*</span>CheckOutService<span class="token punctuation">)</span> <span class="token function">AddProductToCart</span><span class="token punctuation">(</span>p <span class="token operator">*</span>Product<span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var m=t(b,16);l(m,{children:(s,i)=>{var n=j(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>b <span class="token operator">*</span>BookingAppService<span class="token punctuation">)</span> <span class="token function">CreateBooking</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> booking Booking<span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var w=t(m,12);l(w,{children:(s,i)=>{var n=M(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">package</span> chapter4
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>m MailChimp<span class="token punctuation">)</span> <span class="token function">SendEmail</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> to <span class="token builtin">string</span><span class="token punctuation">,</span> title <span class="token builtin">string</span><span class="token punctuation">,</span> body <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token punctuation">&#125;</span>
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var h=t(w,4);l(h,{children:(s,i)=>{var n=U(),a=p(n);c(a,()=>`<code class="language-go"><span class="token keyword">type</span> BookingAppService <span class="token keyword">struct</span> <span class="token punctuation">&#123;</span>
	bookingRepo          BookingRepository
	bookingDomainService BookingDomainService
...
<span class="token keyword">func</span> <span class="token punctuation">(</span>b <span class="token operator">*</span>BookingAppService<span class="token punctuation">)</span> <span class="token function">CreateBooking</span><span class="token punctuation">(</span>ctx context<span class="token punctuation">.</span>Context<span class="token punctuation">,</span> booking Booking<span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">&#123;</span>
	...
	<span class="token keyword">return</span> <span class="token boolean">nil</span>
<span class="token punctuation">&#125;</span></code>`),o(n),e(s,n)},$$slots:{default:!0}});var v=t(h,11),S=t(p(v),2);_(S,{alt:"Domain-Driven Design with Golang Cover",src:"https://learning.oreilly.com/covers/urn:orm:book:9781804613450/400w/"}),D(3),o(v),e(C,r)}export{nn as default,I as metadata};
