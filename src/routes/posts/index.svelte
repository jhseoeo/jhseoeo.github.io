<script context="module">
  ...
  export async function load({ fetch }) {
    const posts = await fetch(`${base}/posts/posts_json`).then((r) => r.json());
...
<div class="wrapper">
  ...
  {#each Object.keys(posts) as mainCategory}
    <div class="category">
      <a class="postname" href={`${base}/posts/${mainCategory}`}
        >{mainCategory}</a
      >
      ...
      {#each Object.keys(posts[mainCategory]) as subCategory}
        <details>
          <summary
            ><a href={`${base}/posts/${mainCategory}/${subCategory}`}
              >{subCategory}</a
            ></summary
          >
          <ul>
            {#each posts[mainCategory][subCategory] as content}
              <li>
                <a
